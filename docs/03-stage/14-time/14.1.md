...
1.  **Topological Ordering (Existence):**
    * *Discrete Premise:* the **acyclic effective causality Axiom** [(§2.7.1)](/monograph/rules/axioms/2.7/#2.7.1) establishes that the causal graph $G$ is a Directed Acyclic Graph (DAG).
...
2.  **Differentiable Structure (Regularity):**
    * *Discrete Premise:* the **Sobolev Convergence Lemma** [(§14.1.5)](/monograph/stage/time/14.1/#14.1.5) establishes that the Lapse function $N^{(t)}$, representing the local density of proper time relative to logical depth, converges in the Sobolev space $H^k(M)$.
