...
* Formulate the Lapse Function <Ref id="14.1.1" label="§14.1.1" /> from the local density of logical graph updates.
* Construct the Lorentzian Metric <Ref id="14.2.1" label="§14.2.1" /> with signature (-+++) under 3+1 foliation.
* Prove Proper Time Maximization <Ref id="14.2.7" label="§14.2.7" /> along geodesic trajectories.
* Verify the Wightman Axioms <Ref id="14.3.2" label="§14.3.2" /> for matter fields residing on the emergent manifold.
* Recover the classical Einstein Field Equations <Ref id="14.4.1" label="§14.4.1" /> from thermodynamic entanglement entropy.
:::
...
1.  **The Lapse Optimization** <Ref id="14.1.3" label="§14.1.3" />: The argument demonstrates that the discrete lapse minimizes a local variance functional, satisfying a discrete Dirichlet problem.
2.  **The Operator Convergence** <Ref id="14.1.5" label="§14.1.5" />: The argument proves the spectral convergence of the discrete Laplacian to the continuous Laplace-Beltrami operator.
3.  **The Elliptic Regularity** <Ref id="14.1.6" label="§14.1.6" />: The argument utilizes elliptic regularity to bootstrap the weak convergence of the lapse into a smooth $C^\infty$ field.
...
Verification of the proper time convergence and lapse smoothness established in the proper time convergence theorem **Local Causal Averages Lemma** <Ref id="14.1.3" label="§14.1.3" /> is based on the following protocols:
...
2.  **Differentiable Structure (Regularity):**
    * *Discrete Premise:* the **Sobolev Convergence Lemma** <Ref id="14.1.5" label="§14.1.5" /> establishes that the Lapse function $N^{(t)}$, representing the local density of proper time relative to logical depth, converges in the Sobolev space $H^k(M)$.
...
3.  **Metric Decomposition (Geometry):**
    ...
    * *Analysis:* the **Lapse function definition** <Ref id="14.1.1" label="§14.1.1" /> verifies that in the intrinsic update frame, the Shift vector vanishes ($\beta^i = 0$).
...
Verification of the global time foliation properties established in the Global Foliation Proof **Global Foliation Proof** <Ref id="14.1.6" label="§14.1.6" /> is based on the following protocols:
