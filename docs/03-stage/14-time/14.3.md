...
The assertion that a discrete, information-theoretic substrate can reproduce the continuous symmetries of the Poincaré group is non-trivial. The proof of **Wightman Compliance Theorem** <Ref id="14.3.2" label="§14.3.2" />is therefore distributed across five specific lemmas, each addressing a core structural requirement of the Wightman formalism:

1.  **Symmetry Recovery (**Poincaré Covariance Lemma** <Ref id="14.3.3" label="§14.3.3" />):** We verify that the statistical isotropy of the graph translates into continuous rotational and boost invariance in the thermodynamic limit.
2.  **Vacuum Stability (**Vacuum Invariance Lemma** <Ref id="14.3.4" label="§14.3.4" />):** We prove that the "Empty Graph" is legally equivalent to the QFT vacuum—a state of zero energy and zero momentum that looks the same in all reference frames.
3.  **Positive Energy (**Spectral Condition Lemma** <Ref id="14.3.5" label="§14.3.5" />):** We demonstrate that because "energy" in this theory corresponds to topological complexity (which is a count of crossings), it is fundamentally bounded below by zero. Negative energy states are topologically impossible.
4.  **Locality (**Microcausality Lemma** <Ref id="14.3.6" label="§14.3.6" />):** We derive the commutativity of fields from the simple fact that graph updates in disconnected components cannot influence each other.
5.  **Spin-Statistics (**Spin-Statistics Relation Lemma** <Ref id="14.3.7" label="§14.3.7" />):** Finally, we verify the deep connection between rotation and exchange, proving that our topological braids naturally obey the exclusion principle required for fermions.
...
**Causal Isomorphism Lemma** <Ref id="14.2.4" label="§14.2.4" />proves that the causal order of the graph maps isomorphically to the conformal structure of the Lorentzian manifold. By the **Alexandrov-Zeeman Theorem**, the group of bijections that preserve the causal order on a Minkowski spacetime is exactly the Poincaré group (plus dilations). Since the physics is defined solely by causal propagation on the graph, the theory must be invariant under the group of causal automorphisms—the Lorentz group $SO(1,3)$.
...
The total energy of a propagating state includes the kinetic term derived from the graph evolution. Since the metric signature is Lorentzian $(-1, +1, +1, +1)$ and the causal propagation speed is bounded by $c=1$ [(**Null Cone Coincidence Lemma** <Ref id="14.2.5" label="§14.2.5" />)](/monograph/stage/time/14.2/#14.2.5), the dispersion relation satisfies:
...
Verification of the spacelike commutator vanishing established in the Microcausality Lemma **Microcausality Lemma** <Ref id="14.3.6" label="§14.3.6" /> is based on the following protocols:
...
the **Spectral Condition Lemma** <Ref id="14.3.5" label="§14.3.5" /> (Spectral Condition) strictly enforces $E \ge 0$ based on the positivity of graph complexity. the **Microcausality Lemma** <Ref id="14.3.6" label="§14.3.6" /> (Microcausality) enforces strict causal independence. Therefore, the substrate axioms physically forbid the "wrong" quantization choices. The system is mathematically forced into the standard Spin-Statistics relation to survive the continuum limit.
...
1.  **Poincaré Covariance ($W_1$):** the **Poincaré Covariance Lemma** <Ref id="14.3.3" label="§14.3.3" /> establishes that the statistical isotropy and homogeneity of the equilibrium graph converge to a unitary representation of the Poincaré group $U(\Lambda, a)$.
2.  **Vacuum Uniqueness ($W_2$):** the **Vacuum Invariance Lemma** <Ref id="14.3.4" label="§14.3.4" /> proves that the maximum entropy state $|0\rangle$ is the unique, invariant ground state of the evolution operator, satisfying $P^\mu |0\rangle = 0$.
3.  **Spectral Condition ($W_3$):** the **Spectral Condition Lemma** <Ref id="14.3.5" label="§14.3.5" /> demonstrates that the identification of mass with topological complexity ($N_3 \ge 0$) strictly confines the energy-momentum spectrum to the forward light cone $\bar{V}^+$, ensuring stability.
4.  **Microcausality ($W_4$):** the **Microcausality Lemma** <Ref id="14.3.6" label="§14.3.6" /> validates that the strict acyclicity of the underlying graph enforces the commutativity of field operators at spacelike separations, preventing superluminal signaling.
5.  **Spin-Statistics ($W_5$):** the **Spin-Statistics Relation Lemma** <Ref id="14.3.7" label="§14.3.7" /> confirms that the topological phases of braid exchange necessitate the assignment of Fermi-Dirac statistics to half-integer spin fields and Bose-Einstein statistics to integer spin fields.
...
Verification of the spatial correlation decay established in the Cluster Decomposition Proof **Spin-Statistics Relation Lemma** <Ref id="14.3.7" label="§14.3.7" /> is based on the following protocols:
