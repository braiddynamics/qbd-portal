...
3.  **The Cone Alignment** <Ref id="14.2.5" label="§14.2.5" />: The argument demonstrates that the metric null boundary corresponds to the maximum speed of information propagation on the graph.
...
The construction of the tetrad field proceeds via the explicit diagonalization of the local metric tensor with respect to the gradient of the global time function defined in **Global Foliation Proof** <Ref id="14.1.6" label="§14.1.6" />.

**I. Temporal Basis Construction**
The zeroth tetrad co-vector $\theta^0$ is defined as the normalized 1-form of the global time gradient. Using the Lapse function $N$ derived in **Lapse Smoothness Theorem** <Ref id="14.1.2" label="§14.1.2" />, the co-vector is $\theta^0_\mu = N \nabla_\mu T$. The corresponding vector field is $e_0^\mu = \frac{1}{N} g^{\mu\nu} \nabla_\nu T$. By the definition of the Lapse as the proper time normalization factor, this vector is strictly unit timelike and future-directed:
...
The emergent metric definition **Emergent Lorentzian Metric** <Ref id="14.2.1" label="§14.2.1" /> implies that for a null vector field $k^\mu$ tangent to a light ray ($ds^2 = 0$), the relationship between spatial displacement and temporal coordinate change is governed by the Lapse function $N$:
...
The Lapse function $N$ is defined in **Lapse function definition** <Ref id="14.1.1" label="§14.1.1" /> as the continuum limit of the ratio of proper distance (edges) to logical depth (ticks). Therefore:
...
In the continuum limit [(**Global Foliation Proof** <Ref id="14.1.6" label="§14.1.6" />)](/monograph/stage/time/14.1/#14.1.6), this depth function maps to a global scalar time field $T: M \to \mathbb{R}$ with a timelike gradient $\nabla T$.
...
1.  **Geometric Instantiation ($Ax1 \to g_{\mu\nu}$):**
    * *Discrete Premise:* The graph Laplacian admits a local spectral decomposition [(**Emergent Tetrad Lemma** <Ref id="14.2.3" label="§14.2.3" />)](/monograph/stage/time/14.2/#14.2.3).
...
2.  **Causal Determinism ($Ax2 \to \Sigma_t$):**
    ...
    * *Continuum Limit:* the **Global Hyperbolicity Lemma** <Ref id="14.2.6" label="§14.2.6" /> proves that the transitive closure of the graph maps to a globally hyperbolic spacetime foliated by Cauchy surfaces $\Sigma_t$.
...
3.  **Kinematic Universality ($Ax3 \to \Gamma^\mu_{\alpha\beta}$):**
    ...
    * *Continuum Limit:* the **Geodesic Motion Lemma** <Ref id="14.2.7" label="§14.2.7" /> establishes that the graph evolution operator $\mathcal{U}$ acts on these defects such that their stationary phase trajectory maximizes proper time $\tau$.
...
Verification of the geodesic emergence and proper time maximization established in the Geodesic Motion Proof **Geodesic Motion Proof** <Ref id="14.2.8" label="§14.2.8" /> is based on the following protocols:
