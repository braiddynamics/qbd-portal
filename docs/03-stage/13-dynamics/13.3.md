...
As established in **Bianchi Error Scaling** <Ref id="13.3.5" label="§13.3.5" />, discrete geometric "leaks" are quadratically suppressed by the lattice scale $\ell_0^2$ and stochastically averaged out by $1/\sqrt{N_t}$. This guarantees that the field equations $\mathcal{G}_{ab} = \kappa T_{ab}$ remain mathematically consistent and free of artificial sources across all scales.
