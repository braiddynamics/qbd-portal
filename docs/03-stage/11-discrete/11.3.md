...
:::note[**the **Monotonicity Theorem** [(§11.3)](/monograph/stage/discrete/11.3/#11.3) Overview**]
:::

The Monotonicity Theorem functions as the conceptual cornerstone for deriving the Emergent Field Equations, providing the mathematical conduit between the discrete computational thermodynamics and the continuous geometry of spacetime. The axioms and dynamical rules of the framework dictate the genesis of 3-cycles, the atomic quanta of geometric information. The master equation and homeostatic equilibrium dictate the proliferation and stabilization of these quanta at a positive density $\rho_3^*$ **equilibrium fixed point** [(§5.4.1)](/monograph/rules/equilibrium/5.4/#5.4.1), constituting the "geometric vacuum." To ascend this combinatorial dynamics to a gravitational theory, the framework demands a rigorous demonstration that the dynamics induce a quantifiable geometric signature. The Monotonicity Theorem supplies this demonstration by establishing that the model's core physical operation equates mathematically to the generation of positive curvature ($\Delta K > 0$) in the causal Ollivier-Ricci metric.
...
### 11.3.1.1 Commentary: The Cost of Curvature {#11.3.1.1}

:::info[**Interpretation of the Action as an Aggregate Transport Score**]
:::

The **discrete einstein-hilbert action definition** [(§11.3.1)](/monograph/stage/discrete/11.3/#11.3.1) of the discrete action **discrete action definition** [(§11.3.1)](/monograph/stage/discrete/11.3/#11.3.1) performs the crucial work of translating the abstract concept of "gravity" into the mechanistic language of information transport. In the continuum of General Relativity, the Einstein-Hilbert action serves as a measure of the total curvature of spacetime, effectively quantifying how much the geometry deviates from flatness. In the discrete regime of Quantum Braid Dynamics, the action $\mathcal{S}$ reinterprets this deviation as a measure of the total "transport efficiency" of the causal graph.
...
1.  **The Measure Dilution** [(§11.3.3)](/monograph/stage/discrete/11.3/#11.3.3): The argument quantifies the reallocation of probability mass, proving the emergence of a non-zero shared mass at the new vertex.
2.  **The Feasible Plan** [(§11.3.4)](/monograph/stage/discrete/11.3/#11.3.4): The argument constructs a valid transport coupling that exploits the shared mass to achieve a zero-cost local transfer.
3.  **The Cost Delimitation** [(§11.3.5)](/monograph/stage/discrete/11.3/#11.3.5): The argument bounds the optimal successor cost term-by-term, establishing that the shared mass strictly reduces the transport burden.
4.  **The Strict Augmentation** [(§11.3.6)](/monograph/stage/discrete/11.3/#11.3.6): The argument synthesizes these stages to prove that the decreased transport cost forces a strict curvature increase.
...
The proof constructs the coupling $\pi_1$ by first decomposing the measures based on the shared mass derived previously **Measure Dilution Lemma** [(§11.3.3)](/monograph/stage/discrete/11.3/#11.3.3), and then defining the transport kernel for each component.
...
The proof proceeds by evaluating the transport cost functional for the hybrid plan $\pi_1$ constructed in the preceding lemma **Transport Feasibility Lemma** [(§11.3.4)](/monograph/stage/discrete/11.3/#11.3.4) and comparing it term-wise to the antecedent cost.
...
Substituting the decomposition $\pi_1 = \pi_{static} + \pi_{rem}$ established previously **Transport Feasibility Lemma** [(§11.3.4)](/monograph/stage/discrete/11.3/#11.3.4):
$$
...
$$
We established in the Measure Dilution Lemma **Measure Dilution Lemma** [(§11.3.3)](/monograph/stage/discrete/11.3/#11.3.3) that the shared mass is strictly positive ($m_w > 0$). Furthermore, in the antecedent sparse graph $G_0$, the neighborhoods are disjoint, implying a non-zero initial transport distance ($W_1^{(0)} > 0$).
...
The true Wasserstein distance $W_1^{(1)}$ is defined as the infimum over all valid couplings $\Pi(\mu_u^{(1)}, \mu_v^{(1)})$. Since $\pi_1$ is a valid coupling (as proven in **Transport Feasibility Lemma** [(§11.3.4)](/monograph/stage/discrete/11.3/#11.3.4)), the optimal cost must be less than or equal to the cost of $\pi_1$:
$$
W_1^{(1)} \le C(\pi_1).
$$
By transitivity:
...
The proof synthesizes the definitions and lemmas established in Phases 1 through 3 to rigorously demonstrate the global monotonicity of the geometric evolution asserted in **Curvature Monotonicity Theorem** [(§11.3.2)](/monograph/stage/discrete/11.3/#11.3.2). We proceed by chaining the logical implications of the mass redistribution, transport feasibility, and cost contraction.

1.  **Mass Redistribution (Phase 1):**
    From ****Measure Dilution Lemma** [(§11.3.3)](/monograph/stage/discrete/11.3/#11.3.3)** [(§11.3.3)](/monograph/stage/discrete/11.3/#11.3.3), we established that the topological nucleation of the 3-cycle involving vertex $w$ necessitates a strictly positive shared probability mass $m_w$ in the successor measures:
    $$
    m_w = \min(\mu_u^{(1)}(w), \mu_v^{(1)}(w)) > 0.
    $$

2.  **Transport Efficiency (Phase 2 & 3):**
    From ****Transport Feasibility Lemma** [(§11.3.4)](/monograph/stage/discrete/11.3/#11.3.4)** [(§11.3.4)](/monograph/stage/discrete/11.3/#11.3.4), we constructed a valid transport coupling $\pi_1$ that utilizes this shared mass. From ****Cost Contraction Lemma** [(§11.3.5)](/monograph/stage/discrete/11.3/#11.3.5)** [(§11.3.5)](/monograph/stage/discrete/11.3/#11.3.5), we proved that the cost of this plan is strictly bounded by the antecedent optimal cost:
    $$
    W_1^{(1)} \le C(\pi_1) < W_1^{(0)}.
    $$

3.  **Curvature Increase:**
    We apply the definition of the Causal Ollivier-Ricci Curvature **Causal Ollivier-Ricci curvature** [(§11.2.2)](/monograph/stage/discrete/11.2/#11.2.2) to the inequality derived above.
    $$
    K^{(1)}(u,v) = 1 - W_1^{(1)}(u,v).
    $$
...
Effects on distant edges vanish due to the exponential decay of correlations **correlation decay lemma** [(§5.1.3)](/monograph/rules/equilibrium/5.1/#5.1.3), limiting the effective radius of the perturbation to $\xi$.
$$
\Delta \mathcal{S} \approx \Delta K_{uv} + \Delta K_{vw} + \Delta K_{wu}.
$$

**III. Curvature Contribution**
From ****Strict Augmentation Proof** [(§11.3.6)](/monograph/stage/discrete/11.3/#11.3.6)**, we have established $\Delta K_{uv} > 0$.
...
### 11.3.7.2 Commentary: The Geometric Quantum {#11.3.7.2}
...
1. **Measure Dilution Lemma** [(§11.3.3)](/monograph/stage/discrete/11.3/#11.3.3)(Shared Mass > 0):   True (m_w = 0.3333)
2. **Cost Contraction Lemma** [(§11.3.5)](/monograph/stage/discrete/11.3/#11.3.5)(W1_post < W1_pre):  True (ΔK = 0.6667)
3. Corollary 11.3.7 (Sparse Scaling): c ≈ 0.4600 (per cycle)
```

The verification confirms the entire proof chain:

1. Measure Dilution: The post-state measures show shared mass at node 2 ($m_w = 0.333$), confirming **Measure Dilution Lemma** [(§11.3.3)](/monograph/stage/discrete/11.3/#11.3.3).
2. Cost Contraction: The Wasserstein distance drops from 0.667 to 0.0, confirming the strict inequality of **Cost Contraction Lemma** [(§11.3.5)](/monograph/stage/discrete/11.3/#11.3.5).
3. Monotonicity: Curvature increases by $\Delta K = 0.667$, verifying the central **Curvature Monotonicity Theorem** [(§11.3.2)](/monograph/stage/discrete/11.3/#11.3.2).
