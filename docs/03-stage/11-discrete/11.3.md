...
The proof proceeds by explicitly constructing the neighborhood sets and applying the **"Tilt" of Time** <Ref id="11.2.1.1" label="§11.2.1.1" /> to the pre-nucleation graph $G_0$ and the post-nucleation graph $G_1$. Let $\alpha, \beta$ be the fixed parameters of the measure, strictly positive (specifically $\alpha=\beta=1/3$).
...
We apply the **"Tilt" of Time** <Ref id="11.2.1.1" label="§11.2.1.1" /> to the updated graph $G_1$.
