...
def lazy_mu(u, G, alpha=1/3, beta=1/3):
    """
    Lazy causal measure μ_u (**Measure Dilution (Phase 1)** <Ref id="11.3.3" label="§11.3.3" />).
...
def w1_linprog(mu_source, mu_target, dist_dict, nodes):
    """
    W_1 via linprog (**Cost Contraction (Phase 3)** <Ref id="11.3.5" label="§11.3.5" />: Cost Contraction).
    """
    n = len(nodes)
    c = []
    inf_indices = []
...
# G0: Chain 0→1→2 
# (**Measure Dilution (Phase 1)** <Ref id="11.3.3" label="§11.3.3" />Pre-state: Disjoint neighborhoods)
...
# G1: Add cycle 2→0 
# (**Measure Dilution (Phase 1)** <Ref id="11.3.3" label="§11.3.3" />Post-state: Shared mass at node 2)
G1 = G0.copy()
G1.add_edge(2, 0)
...
# --- Verification Logic ---
# 1. Verify Shared Mass (**Measure Dilution (Phase 1)** <Ref id="11.3.3" label="§11.3.3" />)
m_w = min(mu0_post.get(2,0), mu1_post.get(2,0))
dilution_verified = (m_w > 0)

# 2. Verify Strict Inequality (**Cost Contraction (Phase 3)** <Ref id="11.3.5" label="§11.3.5" />)
...
print(f"1. **Measure Dilution (Phase 1)** <Ref id="11.3.3" label="§11.3.3" />(Shared Mass > 0):   {dilution_verified} (m_w = {m_w:.4f})")
print(f"2. **Cost Contraction (Phase 3)** <Ref id="11.3.5" label="§11.3.5" />(W1_post < W1_pre):  {contraction_verified} (ΔK = {K_post - K_pre:.4f})")
