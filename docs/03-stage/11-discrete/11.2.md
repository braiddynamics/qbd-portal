...
### 11.2.1.1 Commentary: The "Tilt" of Time {#11.2.1.1}
...
The **causal ollivier-ricci curvature definition** [(§11.2.2)](/monograph/stage/discrete/11.2/#11.2.2) of $K(u,v)$ provides a direct operational interpretation of curvature:
...
1.  **Measure Validity:** For all $u \in V$, the object $\mu_u$ defined in **lazy causal measure definition** [(§11.2.1)](/monograph/stage/discrete/11.2/#11.2.1) is a valid probability measure, satisfying non-negativity and the normalization condition $\sum_{x \in V} \mu_u(x) = 1$.
...
The **causal geometry construction theorem** [(§11.2.3)](/monograph/stage/discrete/11.2/#11.2.3) guarantees that the discrete Einstein-Hilbert action $\mathcal{S}[G] = \sum_{(u,v) \in E} K(u,v)$ is a well-defined functional for any physically realizable state of the causal graph.
...
1.  **The Normalization** [(§11.2.4)](/monograph/stage/discrete/11.2/#11.2.4): The argument verifies probability normalization under the laziness adjustment, preventing mass leakage in vacuum regions.
2.  **The Entropy Maximization** [(§11.2.5)](/monograph/stage/discrete/11.2/#11.2.5): The argument derives the equilibrium parameters from a maximum entropy principle, securing geometric stability.
3.  **The Metric Necessity** [(§11.2.6)](/monograph/stage/discrete/11.2/#11.2.6): The argument demonstrates that undirected distances are required to avoid cost divergences, justifying the metric relaxation.
...
Verification of the probability measure validity established in the Measure Validity Lemma **Measure Validity Lemma** [(§11.2.4)](/monograph/stage/discrete/11.2/#11.2.4) is based on the following protocols:
...
:::info[**The Necessity of Laziness for Numerical Stability**]
:::
...
A valid coupling $\pi$ must transport mass from the support of $\mu_u$ to the support of $\mu_v$. If the topology is tree-like (as in the sparse equilibrium limit **bounded vertex degree lemma** [(§5.5.3)](/monograph/rules/equilibrium/5.5/#5.5.3)), the supports may be disjoint.
...
:::info[**The Necessity of Metric Robustness for Geometric Continuity**]
:::
...
The proof analyzes a minimal directed chain configuration $G = (V, E)$ with $V = \{A, B, C\}$ and edges $E = \{(A,B), (B,C)\}$. The proof fixes the laziness parameters at the entropic optimum $\alpha = 1/3$ and $\beta = 1/3$ **Entropy Maximization Lemma** [(§11.2.5)](/monograph/stage/discrete/11.2/#11.2.5). The undirected shortest-path metric $\bar{d}$ assigns the following values to the vertex pairs:
$$
...
The indicator function $\mathbb{I}[N^-(A) = \emptyset]$ evaluates to 1, triggering the conservation rule defined in **lazy causal measure definition** [(§11.2.1)](/monograph/stage/discrete/11.2/#11.2.1). The mass $\beta$ allocated to the past reassigns to the vertex $A$.
$$
\mu_A(x) = 
\begin{cases} 
...
### 11.2.7.3 Commentary: The Arrow of Time in Static Geometry {#11.2.7.3}
...
The proof of the Causal Geometry Construction Theorem **Causal Geometry Construction Theorem** [(§11.2.3)](/monograph/stage/discrete/11.2/#11.2.3) proceeds by aggregating the independent validation lemmas established in this section. This synthesis confirms that the tuple $(G, \bar{d}, \{\mu_u\}, K)$ constitutes a mathematically rigorous metric measure space capable of supporting a finite, time-oriented curvature calculus.

1.  **Measure Existence and Normalization:**
    **Measure Validity Lemma** [(§11.2.4)](/monograph/stage/discrete/11.2/#11.2.4) guarantees that for every vertex $u \in V$, the object $\mu_u$ constitutes a valid probability measure ($\sum \mu_u(x) = 1$). The explicit handling of vacuum states via the laziness adjustment ensures that no topological configuration results in measure collapse or mass leakage, securing the input stability for the transport functional.

2.  **Metric Finiteness and Stability:**
    **Metric Necessity Lemma** [(§11.2.6)](/monograph/stage/discrete/11.2/#11.2.6) establishes that the undirected shortest-path metric $\bar{d}$ is strictly necessary to prevent divergence. By proving that directed metrics yield infinite transport costs for reverse-time analysis, the **compensation by causal measures lemma** [(§11.2.7)](/monograph/stage/discrete/11.2/#11.2.7) justifies the use of $\bar{d}$ to ensure that $W_1(\mu_u, \mu_v) < \infty$ for all connected pairs, rendering the curvature $K(u,v)$ computable and continuous everywhere.

3.  **Causal Fidelity and Orientation:**
    **Causal Compensation Lemma** [(§11.2.7)](/monograph/stage/discrete/11.2/#11.2.7) demonstrates that the undirected metric does not erase the arrow of time. The proof verifies that the temporal biases encoded in the measures $\mu_u, \mu_v$ (specifically the $\alpha=1/3$ equilibrium derived in **Entropy Maximization Lemma** [(§11.2.5)](/monograph/stage/discrete/11.2/#11.2.5)) sufficiently modulate the transport cost to distinguish forward propagation from reverse propagation. This confirms that $K(u,v)$ encodes the directed causal structure of the underlying graph $G$.
