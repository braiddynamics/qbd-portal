...
1.  **The Normalization** <Ref id="11.2.4" label="§11.2.4" />: The argument verifies probability normalization under the laziness adjustment, preventing mass leakage in vacuum regions.
...
Verification of the probability measure validity established in the Measure Validity Lemma **Measure Validity Lemma** <Ref id="11.2.4" label="§11.2.4" /> is based on the following protocols:
...
The proof analyzes a minimal directed chain configuration $G = (V, E)$ with $V = \{A, B, C\}$ and edges $E = \{(A,B), (B,C)\}$. The proof fixes the laziness parameters at the entropic optimum $\alpha = 1/3$ and $\beta = 1/3$ **Entropy Maximization Lemma** <Ref id="11.2.5" label="§11.2.5" />. The undirected shortest-path metric $\bar{d}$ assigns the following values to the vertex pairs:
...
def lazy_mu_dynamic(u, G, alpha=1.0/3.0, beta=1.0/3.0):
    ...
    Implements the Re-absorption Logic (**Measure Validity Lemma** <Ref id="11.2.4" label="§11.2.4" />).
    """
    N_plus = list(G.successors(u))
    N_minus = list(G.predecessors(u))
    n_plus = len(N_plus)
