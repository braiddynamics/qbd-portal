...
By the hypothesis of the **Discrete Field Equations** **Discrete Bianchi Identity** <Ref id="12.3" label="§12.3" />, the discrete fields satisfy the relation $\mathcal{G}^{(t)}_e - \kappa \mathcal{T}^{(t)}_e = 0$ for every edge. Applying the linear operator $\mathcal{A}_R^{(t)}$ to this null field:
