...
The **Riemann Sum Approximation** <Ref id="13.2.4" label="§13.2.4" /> provides the "compilation instruction" for translating discrete graph data into continuum geometry. It answers a fundamental question: How does a simple number on an edge (like flux or curvature) become a tensor that defines distances and angles?
...
By the hypothesis of the discrete field equations (specifically, **Geometric Conservation** <Ref id="12.3" label="§12.3" />), the discrete fields satisfy the relation $\mathcal{G}^{(t)}_e - \kappa \mathcal{T}^{(t)}_e = 0$ for every edge. Applying the linear operator $\mathcal{A}_R^{(t)}$ to this null field:
