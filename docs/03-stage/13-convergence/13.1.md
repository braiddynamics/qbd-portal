...
:::note[**the **Smooth Manifold Limit Section** [(§13.1)](/monograph/stage/convergence/13.1/#13.1) Overview**]
:::
...
| :--- | :--- | :--- | :--- |
| **Uniform Sampling** | Density bounded away from 0 and $\infty$ | **Ahlfors 4-Regularity** enforces bounded volume growth $|B(r)| \sim r^4$. | **emergent Hausdorff dimension** [(§5.5.7)](/monograph/rules/equilibrium/5.5/#5.5.7) |
...
| **Bounded Curvature** | Ricci curvature bounded below | **Geometric Syndrome** enforces $|K| \leq 2$. | **Curvature Monotonicity Theorem** [(§11.3.2)](/monograph/stage/discrete/11.3/#11.3.2) |
...
From ****Spectral Convergence Lemma** [(§13.1.3)](/monograph/stage/convergence/13.1/#13.1.3)** [(§13.1.3)](/monograph/stage/convergence/13.1/#13.1.3), the discrete eigenfunctions converge to limit functions $f_k \in L^2(M)$ which satisfy the weak eigenvalue equation for the Laplace-Beltrami operator:
$$
...
The equation $-\Delta_g f_k - \lambda_k f_k = 0$ constitutes a linear, second-order, uniformly elliptic partial differential equation. The **Interior Regularity Theorem** for elliptic operators **[(Gilbarg & Trudinger, 2001, Thm 9.11)](/monograph/appendices/a-references#A.75)** states:
...
The **Sobolev Embedding Theorem** **[(Adams & Fournier, 2003)](/monograph/appendices/a-references#A.4)** establishes the injection of Sobolev spaces into spaces of continuous derivatives. Specifically, for a manifold of dimension $d=4$:
$$
...
From ****Spectral Convergence Lemma** [(§13.1.3)](/monograph/stage/convergence/13.1/#13.1.3)**, the sequence of consistently weighted Laplacians $\{\tilde{\mathcal{L}}_t\}$ converges to the continuum Laplace-Beltrami operator $-\Delta_g$ in the sense of strong resolvent convergence. This implies two critical convergences as $N_t \to \infty$:
...
****Heat Kernel Lemma** [(§13.1.4)](/monograph/stage/convergence/13.1/#13.1.4)** establishes that the heat kernel $p_t(x,y)$ of the limit space admits short-time Gaussian bounds characteristic of a 4-dimensional Euclidean space.
$$
...
The limit eigenfunctions $\{f_k\}_{k=1}^\infty$ form a complete orthonormal basis for $L^2(M)$. From ****Smoothness Lemma** [(§13.1.5)](/monograph/stage/convergence/13.1/#13.1.5)**, these functions are $C^\infty$-smooth. We define the **Spectral Embedding** map $\Phi_K: M \to \mathbb{R}^K$ by:
$$
