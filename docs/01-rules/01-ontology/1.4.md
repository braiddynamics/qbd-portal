...
### 1.4.2.1 Commentary: Causal Construction {#1.4.2.1}
...
### 1.4.3.1 Commentary: Causal Destruction {#1.4.3.1}
...
$\mathfrak{T}_{del}$ is defined as a topological modification, not an informational erasure. Within the Elementary Task Space, the excision of a causal link $e$ removes the *active relation* (causal influence) but does not retroactively annihilate the *event of its creation*. The task space assumes an "Append-Only" metaphysics regarding the Global Sequencer's log: $t_L$ at which $e$ was created remains a persistent property of the universe's trajectory, even if the geometric constituent $e$ is removed from the active graph $G$. This distinction allows for the pruning of geometry without the paradox of altering the past. Critically, this append-only historical poset (**Monotonicity of History** <Ref id="1.3.5" label="§1.3.5" />) incurs zero runtime memory overhead; when $e$ is pruned by $\mathfrak{T}_{del}$ (as established by the **Vacuum Repertoire** <Ref id="1.4.4" label="§1.4.4" />), it is fully excised from the active state graph data structure, maintaining strict structural sparsity and computational efficiency without retaining inactive or historically deleted edges in memory.
...
### 1.4.4.1 Commentary: Argument Outline {#1.4.4.1}
...
```text
• 1.4.4 Theorem The Vacuum Repertoire  [by construction]
├── 1.4.4.2 Diagram Task Repertoire
│
├── 1.4.5 Lemma Relational Vertex Emergence
│   ├── 1.4.5.1 Proof Relational Vertex Emergence
│   └── 1.4.5.2 Commentary Ontological Minimality
│
├── 1.4.6 Lemma Reversibility of Primitives
│   ├── 1.4.6.1 Proof Reversibility of Primitives
│   └── 1.4.6.2 Commentary Constructor Reversibility
│
└── 1.4.7 Proof The Vacuum Repertoire
```

---

### 1.4.4.2 Diagram: Task Repertoire {#1.4.4.2}
...
Let $T \in \mathfrak{T}_{vac}$ be a primitive task. By **Edge Addition Task** <Ref id="1.4.2" label="§1.4.2" /> and **Edge Deletion Task** <Ref id="1.4.3" label="§1.4.3" />, the mapping $T: G \mapsto G'$ satisfies:
...
### 1.4.5.2 Commentary: Ontological Minimality {#1.4.5.2}
...
### 1.4.6.2 Commentary: Constructor Reversibility {#1.4.6.2}
...
By **Relational Vertex Emergence** <Ref id="1.4.5" label="§1.4.5" />, the vertex set is invariant under each primitive operation ($V_{i+1} = V_i$).  Thus:
...
By **Reversibility of Primitives** <Ref id="1.4.6" label="§1.4.6" />, each step $T_i$ possesses a unique inverse task $T_i^{-1}$.  Therefore, the entire sequence $T_m \circ \dots \circ T_1$ is invertible, preserving state distinguishability:
...
This kinematic freedom is necessary but insufficient. While the ability to add and delete edges provides the vocabulary of change, it does not provide the grammar. A universe that can do anything at random will likely do nothing coherent. The verbs of the physical language, which are the creation and destruction of relations, are defined. However, the sentences are not yet defined. The vocabulary of shapes that these simple additions and deletions can form must be understood. It is necessary to know which of those shapes represent valid physical structures versus mathematical noise. The definition of the fundamental topological structures is the next focus.
