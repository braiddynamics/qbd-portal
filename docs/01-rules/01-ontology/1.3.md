...
### 1.3.1.1 Commentary: State Space Snapshots {#1.3.1.1}
...
### 1.3.3.1 Commentary: Irreducible Influence {#1.3.3.1}
...
### 1.3.4.1 Commentary: Temporal Discreteness {#1.3.4.1}
...
### 1.3.4.2 Diagram: Timestamp Evolution {#1.3.4.2}
...
### 1.3.5.1 Commentary: Argument Outline {#1.3.5.1}
...
```text
• 1.3.5 Theorem Monotonicity of History  [by induction]
├── 1.3.5.2 Diagram Causal Cone
│
├── 1.3.6 Lemma Irreflexivity of Timestamps
│   ├── 1.3.6.1 Proof Irreflexivity of Timestamps
│   └── 1.3.6.2 Commentary Loop Exclusion
│
├── 1.3.7 Lemma Transitive Causal Monotonicity
│   ├── 1.3.7.1 Proof Transitive Causal Monotonicity
│   └── 1.3.7.2 Commentary Lamport Ordering
│
└── 1.3.8 Proof Monotonicity of History
```

---

### 1.3.5.2 Diagram: Causal Cone {#1.3.5.2}
...
### 1.3.6.2 Commentary: Loop Exclusion {#1.3.6.2}
...
This structure maintains a rigorous distinction between the event-level Causal History Graph (a strict Directed Acyclic Graph (DAG) by **Causal Ordering** <Ref id="2.2.1" label="§2.2.1" />) and the instantaneous Spatial State Graph $G_t$, which is tiled with directed 3-cycles representing geometric area. Because these spatial 3-cycles do not represent chronological loops in the event poset, spatial geometric triangles form without violating global causal acyclicity (**Geometric Quantum** <Ref id="2.3.2" label="§2.3.2" />).
...
Assume the inequality holds for a path of length $n$: $H(e_{n-1}) < H(e_n)$.  Consider the adjacent edge $e_{n+1} = (v_n, v_{n+1})$ originating at $v_n$.

Since $e_n \in \text{In}(v_n)$, the assignment for $H(e_{n+1})$ satisfies:
...
### 1.3.7.2 Commentary: Lamport Ordering {#1.3.7.2}
...
1.  **Length $k=1$**: Under this condition, the cycle is a self-loop $e = (v_0, v_0)$.  By **Irreflexivity of Timestamps** <Ref id="1.3.6" label="§1.3.6" />, no stable timestamp assignment can exist for a self-loop, generating a contradiction.
2.  **Length $k \ge 2$**: Under this condition, the cycle forms a directed path from $v_0$ to $v_k$.  By **Transitive Causal Monotonicity** <Ref id="1.3.7" label="§1.3.7" />, the edge timestamps must satisfy:
