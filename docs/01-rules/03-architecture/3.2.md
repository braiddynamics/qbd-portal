...
1. **The Pre-Geometric Exclusions (Lemma [§3.2.2], Lemma [§3.2.3]):** The argument eliminates all graph candidates containing cyclic topologies or short-range loops, ensuring the initial state remains strictly pre-geometric and empty of pre-existing area.
2. **The Sparsity and Connectivity Filters (Lemma [§3.2.4], Lemma [§3.2.5]):** The argument excludes disconnected states and redundant acyclic graphs, restricting the candidate space to minimally connected directed tree topologies.
3. **The Homogeneity Optimization (Lemma [§3.2.6], Lemma [§3.2.7], Lemma [§3.2.8]):** The argument enforces orbit transitivity, site maximality, and degree regularity, isolating the regular Bethe fragment as the sole structure that maximizes symmetric homogeneity.
4. **The Metric Convergence (Lemma [§3.2.9], Theorem [§3.2.10], Proof [§3.2.11]):** The argument evaluates the remaining candidates against the structural optimality metric, proving the quantitative supremacy of the regular tree configuration over all alternative graphs.
...
For any graph containing a self-loop or a reciprocal 2-cycle, candidacy for the vacuum state $G_0$ is excluded by the **Directed Causal Link** definition [(§2.1.1)](/monograph/rules/axioms/2.1/#2.1.1).
...
For any tree graph yielding a strictly sub-maximal number of compliant 2-Path rewrite sites [(§1.5.2)](/monograph/rules/ontology/1.5/#1.5.2), candidacy for the vacuum state $G_0$ is excluded. In particular, site maximization constitutes a necessary condition for geometric evolution.
...
Investigation of the entropic properties of regular versus irregular structures established in the Regularity Mandate Proof [(§3.2.7.1)](/monograph/rules/architecture/3.2/#3.2.7.1) is based on the following protocols:
...
For any tree graph where the automorphism group fails to act transitively on vertex levels, candidacy for the vacuum state $G_0$ is excluded by the condition of relational uniformity [(§3.2.9)](/monograph/rules/architecture/3.2/#3.2.9). In particular, level-transitivity constitutes a necessary condition for the absence of privileged positions within each generation.
...
Quantification of the scaling behavior of the Bethe fragment established in the Regularity Mandate Proof [(§3.2.7.1)](/monograph/rules/architecture/3.2/#3.2.7.1) is based on the following protocols:
...
2.  **Interaction Maximization (Lemma [§3.2.6](/monograph/rules/architecture/3.2/#3.2.6)):** To maximize the rate of geometric evolution, the tree structure must maximize the density of compliant 2-path sites per vertex. This requirement favors maximal branching over linear extension.
3.  **Symmetry Maximization (Lemma [§3.2.8](/monograph/rules/architecture/3.2/#3.2.8)):** To prevent the emergence of privileged spatial locations or preferred directions, the graph must exhibit **Level Transitivity** in its automorphism group. This enforces structural regularity, requiring $k$ to be constant across all internal nodes.
