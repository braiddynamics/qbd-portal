...
1. **The Pre-Geometric Exclusions (the **exclusion of cyclic topologies lemma** [(§3.2.2)](/monograph/rules/architecture/3.2/#3.2.2), the **exclusion of short-range loops lemma** [(§3.2.3)](/monograph/rules/architecture/3.2/#3.2.3)):** The argument eliminates all graph candidates containing cyclic topologies or short-range loops, ensuring the initial state remains strictly pre-geometric and empty of pre-existing area.
2. **The Sparsity and Connectivity Filters (the **exclusion of disconnected states lemma** [(§3.2.4)](/monograph/rules/architecture/3.2/#3.2.4), the **exclusion of redundant dags lemma** [(§3.2.5)](/monograph/rules/architecture/3.2/#3.2.5)):** The argument excludes disconnected states and redundant acyclic graphs, restricting the candidate space to minimally connected directed tree topologies.
3. **The Homogeneity Optimization (the **site maximality lemma** [(§3.2.6)](/monograph/rules/architecture/3.2/#3.2.6), the **degree regularity lemma** [(§3.2.7)](/monograph/rules/architecture/3.2/#3.2.7), the **orbit transitivity lemma** [(§3.2.8)](/monograph/rules/architecture/3.2/#3.2.8)):** The argument enforces orbit transitivity, site maximality, and degree regularity, isolating the regular Bethe fragment as the sole structure that maximizes symmetric homogeneity.
4. **The Metric Convergence (the **structural optimality metric lemma** [(§3.2.9)](/monograph/rules/architecture/3.2/#3.2.9), the **quantitative supremacy theorem** [(§3.2.10)](/monograph/rules/architecture/3.2/#3.2.10), the **demonstration of the optimal vacuum proof** [(§3.2.11)](/monograph/rules/architecture/3.2/#3.2.11)):** The argument evaluates the remaining candidates against the structural optimality metric, proving the quantitative supremacy of the regular tree configuration over all alternative graphs.
...
For any graph containing a directed cycle of length greater than or equal to 3, candidacy for the vacuum state $G_0$ is **excluded** [(§2.3.1)](/monograph/rules/axioms/2.3/#2.3.1).
...
The **Axiom of Geometric Constructibility** [(§2.3.1)](/monograph/rules/axioms/2.3/#2.3.1) mandates that the vacuum state remains strictly pre-geometric.
...
For any graph containing a self-loop or a reciprocal 2-cycle, candidacy for the vacuum state $G_0$ is excluded by the **Directed Causal Link** the **irreflexivity axiom** [(§2.1.1)](/monograph/rules/axioms/2.1/#2.1.1).
...
The **Directed Causal Link** [(§2.1.1)](/monograph/rules/axioms/2.1/#2.1.1) mandates that every directed causal link satisfies strict irreflexivity and asymmetry.
...
Both structures constitute primitive geometric cycles existing prior to the application of the rewrite rule. We conclude that all such primitive cycles are excluded from the vacuum state by the **Principle of Unique Causality** [(§2.3.3)](/monograph/rules/axioms/2.3/#2.3.3).
...
For all disconnected graphs, candidacy for the vacuum state $G_0$ is **excluded** [(§2.7.1)](/monograph/rules/axioms/2.7/#2.7.1). In particular, automorphism entropy is minimal and a single interacting universe exists.
...
The **Acyclic Effective Causality** [(§2.7.1)](/monograph/rules/axioms/2.7/#2.7.1) requires that the effective influence relation $\le$ forms a single strict partial order on the entire vertex set $V_0$. This order must exhibit irreflexivity, asymmetry, and transitivity across all vertices simultaneously.
...
For any connected DAG with edge count strictly greater than $N-1$, candidacy for the vacuum state $G_0$ is excluded by the **Principle of Unique Causality** [(§2.3.3)](/monograph/rules/axioms/2.3/#2.3.3).
...
In the directed limit, any connected DAG with $|E| > N-1$ necessitates redundant directed paths between vertex pairs. The **Principle of Unique Causality** [(§2.3.3)](/monograph/rules/axioms/2.3/#2.3.3) excludes redundant causal paths of length $\le 2$. Such redundancies reduce the fraction of compliant 2-path sites available for the rewrite rule below the maximum value of 1.

**III. Probabilistic Decay of Compliance**

Let $\rho = (|E| - N + 1)/N$ denote the redundancy density. The **Axiom of Geometric Constructibility** [(§2.3.1)](/monograph/rules/axioms/2.3/#2.3.1) requires that the vacuum state maximizes the density of compliant rewrite sites. The probability $\mathbb{P}$ that a potential 2-path site remains non-compliant scales as:
...
For any tree graph yielding a strictly sub-maximal number of compliant 2-Path **sites rewrite** [(§1.5.2)](/monograph/rules/ontology/1.5/#1.5.2), candidacy for the vacuum state $G_0$ is excluded. In particular, site maximization constitutes a necessary condition for geometric evolution.
...
The **Principle of Unique Causality** [(§2.3.3)](/monograph/rules/axioms/2.3/#2.3.3) and the **Axiom of Geometric Constructibility** [(§2.3.1)](/monograph/rules/axioms/2.3/#2.3.1) jointly necessitate sufficient participancy of all vertices in the emergent geometric process. This requirement implies the absolute maximum possible number of compliant 2-path sites per vertex.
...
For any non-regular tree graph, candidacy for the vacuum state $G_0$ is excluded by the requirement for maximal **entropy orbit maximal**[(§3.2.9)](/monograph/rules/architecture/3.2/#3.2.9).
...
The uniformity requirements of the **Directed Causal Link** [(§2.1.1)](/monograph/rules/axioms/2.1/#2.1.1) and **Acyclic Effective Causality** [(§2.7.1)](/monograph/rules/axioms/2.7/#2.7.1) necessitate the maximization of this entropy measure. Furthermore, internal degrees less than 3 yield insufficient compliant sites in accordance with previous lemmas.
...
Investigation of the entropic properties of regular versus irregular structures established in the **degree regularity proof** [(§3.2.7.1)](/monograph/rules/architecture/3.2/#3.2.7.1) is based on the following protocols:
...
For any tree graph where the automorphism group fails to act transitively on vertex levels, candidacy for the vacuum state $G_0$ is excluded by the condition of **uniformity relational** [(§3.2.9)](/monograph/rules/architecture/3.2/#3.2.9). In particular, level-transitivity constitutes a necessary condition for the absence of privileged positions within each generation.
...
The vacuum must be maximally symmetric, meaning it should not contain any information that allows an observer to say "I am on the special branch." Everyone at generation $N$ should see the exact same causal horizon, indistinguishable from any other observer at the same generation. The **orbit transitivity lemma** [(§3.2.8)](/monograph/rules/architecture/3.2/#3.2.8) forces the tree to be **Balanced**: every branch must look exactly like every other branch. This symmetry is the discrete precursor to the **Cosmological Principle** (homogeneity and isotropy), ensuring that the laws of physics do not vary depending on which "branch" of the universe you inhabit. The vacuum effectively hides its own history, appearing identical in all directions from the perspective of any internal observer.
...
**The Regular Bethe Fragment** [(§3.2.1)](/monograph/rules/architecture/3.2/#3.2.1) constitutes the unique maximizer of the Structural Optimality Score $\mathcal{O}(G; \lambda)$ over the class of axiomatically admissible graphs for the parameter range $\lambda \in [0.4, 0.6]$.
...
The class of axiomatically admissible graphs reduces, through the cumulative exclusions of the previous lemmas, to the singleton containing the **Regular Bethe Fragment** [(§3.2.1.1)](/monograph/rules/architecture/3.2/#3.2.1.1) with internal coordination number $k_{deg} \ge 3$.
...
Sequential application of the lemma filters and explicit computation of the **Structural Optimality Score** [(§3.2.9)](/monograph/rules/architecture/3.2/#3.2.9) confirms the maximum.
...
Verification of the Regular Bethe Fragment as the unique maximizer established in the **supremacy verification proof** [(§3.2.10.1)](/monograph/rules/architecture/3.2/#3.2.10.1) is based on the following protocols:
...
Quantification of the scaling behavior of the Bethe fragment established in the **degree regularity proof** [(§3.2.7.1)](/monograph/rules/architecture/3.2/#3.2.7.1) is based on the following protocols:
...
:::tip[**Formal Derivation of the Regular Bethe Fragment (k=3) from the **Intersection of Constraints** [(§3.2.1)](/monograph/rules/architecture/3.2/#3.2.1)**]
:::

**I. The Candidate Set**
The set of candidate vacuum states is restricted to the class of Finite Rooted Trees, as established by the **demonstration of the vacuum structure proof** [(§3.1.12)](/monograph/rules/architecture/3.1/#3.1.12). The proof seeks to identify the specific tree topology that maximizes the physical potential for geometrogenesis.
...
2.  **Interaction Maximization** [(§3.2.6)](/monograph/rules/architecture/3.2/#3.2.6): To maximize the rate of geometric evolution, the tree structure must maximize the density of compliant 2-path sites per vertex. This requirement favors maximal branching over linear extension.
3.  **Symmetry Maximization** [(§3.2.8)](/monograph/rules/architecture/3.2/#3.2.8): To prevent the emergence of privileged spatial locations or preferred directions, the graph must exhibit **Level Transitivity** in its automorphism group. This enforces structural regularity, requiring $k$ to be constant across all internal nodes.
