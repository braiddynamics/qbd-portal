...
4. **The Metric Convergence (the structural optimality metric lemma [§3.2.9], the quantitative supremacy theorem [§3.2.10], the optimal vacuum proof [§3.2.11]):** The argument evaluates the remaining candidates against the structural optimality metric, proving the quantitative supremacy of the regular tree configuration over all alternative graphs.
...
For any non-regular tree graph, candidacy for the vacuum state $G_0$ is excluded by the requirement for maximal **orbit entropy** [(§3.2.9)](/monograph/rules/architecture/3.2/#3.2.9).
...
:::tip[the **Formal Derivation of the Regular Bethe Fragment (k=3) from the Intersection of Constraints** [(§3.2.1)](/monograph/rules/architecture/3.2/#3.2.1)]
:::
