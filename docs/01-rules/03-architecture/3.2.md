...
For any tree graph yielding a strictly sub-maximal number of compliant **2-Path** <Ref id="1.5.5" label="§1.5.5" /> rewrite sites, candidacy for the vacuum state $G_0$ is excluded. In particular, site maximization constitutes a necessary condition for geometric evolution.
