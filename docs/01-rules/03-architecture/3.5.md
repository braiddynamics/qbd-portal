...
2.  **The Hard Constraint Projectors:**
    ...
    * **$2$-Cycle Projector:** For every unordered pair of vertices $\{u, v\}$, the operator $\Pi_{\text{cycle}}(u, v)$ prohibits **edges reciprocal prohibits**[(§1.5.3)](/monograph/rules/ontology/1.5/#1.5.3):

        $$
        \Pi_{\text{cycle}}(u, v) = I - \frac{1}{4}(I - Z_{uv})(I - Z_{vu})
        $$

    * **Locality Projector:** For every ordered pair $(u, v)$ where the undirected distance satisfies $\bar{d}(u, v) > 2$, the operator $\Pi_{\text{local}}(u, v)$ prohibits **instantiation edge prohibits**[(§5.5.2)](/monograph/rules/equilibrium/5.5/#5.5.2):
...
Verification of the spectral properties of geometric stabilizers established in the Projector Validity the **projector validity proof** [(§3.5.4.1)](/monograph/rules/architecture/3.5/#3.5.4.1) is based on the following protocols:
...
Generation of the diagnostic lookup tables established in the Classification Validity the **syndrome classification of triplet configurations proof** [(§3.5.5.1)](/monograph/rules/architecture/3.5/#3.5.5.1) is based on the following protocols:
...
:::tip[**Formal Proof of the Equivalence between Causal **Consistency and Quantum Error Correction** [(§3.5.2)](/monograph/rules/architecture/3.5/#3.5.2)**]
:::
...
Verification of the codespace projection and syndrome extraction established in the Isomorphism the **demonstration of the stabilizer isomorphism proof** [(§3.5.8)](/monograph/rules/architecture/3.5/#3.5.8) is based on the following protocols:
