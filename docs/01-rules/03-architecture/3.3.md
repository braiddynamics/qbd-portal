...
1. **The Sufficiency (Lemma [§3.3.4]):** The argument establishes that a maximally parallel update preserves graph automorphisms, utilizing the equivariance of the site definition to guarantee that symmetric inputs yield symmetric outputs.
2. **The Conflict Resolution (Lemma [§3.3.5]):** The argument verifies that symmetry is preserved under overlapping update sites, demonstrating that equivariant conflict resolution rules prevent symmetry-breaking coordination collapses.
3. **The Necessity (Proof [§3.3.7]):** The argument demonstrates that any sequential or non-maximal scheduling of updates introduces distinguishing selections that partition symmetric orbits, proving that partial parallelism collapses the automorphism group.
...
Analysis of the scheduler's impact on vacuum symmetry established in the Overlap Determinism Proof [(§3.3.5.1)](/monograph/rules/architecture/3.3/#3.3.5.1) is based on the following protocols:
...
Assume the graph remains in the sparse regime [(§3.1.2)](/monograph/rules/architecture/3.1/#3.1.2) subject to quasi-local constraints [(§2.3.3)](/monograph/rules/axioms/2.3/#2.3.3) with a bounded check radius $R \propto \log N$. Then the time complexity of the maximally parallel update operation is bounded by $O(\log N)$. Moreover, the probability of conflict chains spanning the system decays exponentially.
...
The vacuum state $G_0$ is defined by maximal symmetry [(§3.2.1)](/monograph/rules/architecture/3.2/#3.2.1).
