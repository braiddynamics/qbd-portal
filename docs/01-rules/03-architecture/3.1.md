...
5.  **Acyclicity:** The graph contains no **Cycle** <Ref id="1.5.6" label="§1.5.6" /> and no redundant **parallel paths** <Ref id="2.3.3" label="§2.3.3" />.
...
Thus, these configurations are strictly **excluded** <Ref id="2.1.1" label="§2.1.1" />.
...
6.  **Bi-Directional Flows** <Ref id="3.1.12" label="§3.1.12" />: Filtered by **Asymmetry**, mandating a single source vertex (the Root) with strictly divergent flow.
