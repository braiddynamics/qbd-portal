...
1.  **Logical Time ($t_L$):** A discrete, monotonically increasing sequence $\mathbb{N}_0$ indexing the evolution of the **graph** [(§1.2.1)](/monograph/rules/ontology/1.2/#1.2.1).
2.  **The History Mapping ($H$):** A function $H: E \to \mathbb{N}$ assigning a strictly immutable creation timestamp to every edge, enforcing the order of **operations** [(§1.3.1)](/monograph/rules/ontology/1.3/#1.3.1).
3.  **Fundamental Graph Structures:**
      * **Directed Acyclic Graph (DAG):** A graph containing no **cycles directed no**[(§1.5.1)](/monograph/rules/ontology/1.5/#1.5.1).
      * **Bipartite Graph:** A graph where vertices define two disjoint sets ($V_A, V_B$) with edges strictly connecting $V_A$ to $V_B$ **fundamental graph structures definition** [(§1.5.1)](/monograph/rules/ontology/1.5/#1.5.1).
      * **The 2-Path:** A simple directed path of length 2 ($v \to w \to u$), serving as the minimal unit of **mediation transitive** [(§1.5.2)](/monograph/rules/ontology/1.5/#1.5.2).
4.  **Axiom 1 (Causal Primitive):** The directed edge $u \to v$ is strictly irreflexive and **asymmetric** [(§2.1.1)](/monograph/rules/axioms/2.1/#2.1.1).
5.  **Axiom 2 (Geometric Primitive):** Valid physical geometry forms exclusively via the closure of directed 3-cycles **geometric constructibility axiom** [(§2.3.1)](/monograph/rules/axioms/2.3/#2.3.1).
6.  **Axiom 3 (Acyclic Effective Causality):** The effective influence relation $\le$ forms a strict partial order on the **vertices** [(§2.7.1)](/monograph/rules/axioms/2.7/#2.7.1).
7.  **Principle of Unique Causality:** Information cannot be cloned: specific paths must be unique to serve as valid candidates for **interaction** [(§2.3.3)](/monograph/rules/axioms/2.3/#2.3.3).
...
5.  **Acyclicity:** The graph contains no **Directed Cycles** [(§1.5.3)](/monograph/rules/ontology/1.5/#1.5.3) and no redundant **paths parallel redundant**[(§2.3.3)](/monograph/rules/axioms/2.3/#2.3.3).
This structure constitutes the unique topological solution compatible with the simultaneous enforcement of the **Causal Primitive** [(§2.1.1)](/monograph/rules/axioms/2.1/#2.1.1), **Geometric Constructibility** [(§2.3.1)](/monograph/rules/axioms/2.3/#2.3.1), and **Acyclic Effective Causality** [(§2.7.1)](/monograph/rules/axioms/2.7/#2.7.1).
...
1. **The Foundation** [(§3.1.4)](/monograph/rules/architecture/3.1/#3.1.4): The argument establishes graph existence and finiteness, proving that the initial vertex set must be finite to satisfy well-foundedness of the causal order.
2. **The Filter (the **exclusion of reflexivity and reciprocity lemma** [(§3.1.5)](/monograph/rules/architecture/3.1/#3.1.5), the **exclusion of cyclic paths lemma** [(§3.1.6)](/monograph/rules/architecture/3.1/#3.1.6), the **global acyclicity lemma** [(§3.1.7)](/monograph/rules/architecture/3.1/#3.1.7)):** The argument systematically excludes all cyclic topologies, proving that reflexivity, reciprocity, and higher-order loops violate causal constraints and restricting the vacuum to a directed acyclic graph.
3. **The Topology (the **global connectivity lemma** [(§3.1.8)](/monograph/rules/architecture/3.1/#3.1.8), the **path uniqueness and sparsity lemma** [(§3.1.9)](/monograph/rules/architecture/3.1/#3.1.9)):** The argument enforces tree sparsity, demonstrating that any excess edge density creates redundant paths that violate the Principle of Unique Causality.
4. **The Bipartition (the **depth-parity bipartition lemma** [(§3.1.10)](/monograph/rules/architecture/3.1/#3.1.10), the **exclusion of odd cycles lemma** [(§3.1.11)](/monograph/rules/architecture/3.1/#3.1.11), the **demonstration of the vacuum structure proof** [(§3.1.12)](/monograph/rules/architecture/3.1/#3.1.12)):** The argument identifies the depth-parity bipartition, proving that odd-length cycles are topologically suppressed and isolating the directed tree as the unique initial state.
...
Let the universe possess an initial state $G_0$ at logical time $t_L = 0$ **temporal finitude theorem** [(§1.2.7)](/monograph/rules/ontology/1.2/#1.2.7). Then the vertex set $V_0$ is finite, and the existence of infinite descending causal chains is **excluded** [(§2.6.1)](/monograph/rules/axioms/2.6/#2.6.1).
...
Let the logical time domain satisfy $T_L \cong \mathbb{N}_0$ **dual time architecture postulate** [(§1.2.1)](/monograph/rules/ontology/1.2/#1.2.1). Let the Effective Influence relation $\le$ constitute a strict partial order on the vertex set $V$ **acyclic effective causality axiom** [(§2.7.1)](/monograph/rules/axioms/2.7/#2.7.1). A strict partial order satisfies well-foundedness if and only if every non-empty subset contains a minimal element.
...
Let $G_0$ denote the initial state of the **universe** [(§1.2.7)](/monograph/rules/ontology/1.2/#1.2.7). Then the existence of **Self-Loops** [(§2.2.2)](/monograph/rules/axioms/2.2/#2.2.2) and reciprocal edge pairs forming 2-**Cycles** [(§1.5.3)](/monograph/rules/ontology/1.5/#1.5.3) is **excluded** [(§2.1.1)](/monograph/rules/axioms/2.1/#2.1.1).
...
Let **The Directed Causal Link** define the elementary relation as strictly irreflexive and **asymmetric** [(§2.1.1)](/monograph/rules/axioms/2.1/#2.1.1).
...
This relation violates the condition of Irreflexivity enforced by **Acyclic Effective Causality** [(§2.7.1)](/monograph/rules/axioms/2.7/#2.7.1).
...
The **Principle of Unique Causality** restricts the creation of geometric cycles exclusively to the rewrite rule $\mathcal{R}$ **principle of unique causality axiom** [(§2.3.3)](/monograph/rules/axioms/2.3/#2.3.3). Pre-existing cycles of length $L=1$ or $L=2$ constitute geometric anomalies preceding dynamical evolution.
...
The **exclusion of reflexivity and reciprocity lemma** [(§3.1.5)](/monograph/rules/architecture/3.1/#3.1.5) systematically eliminates the two most trivial forms of causal paradox: the "Mirror" (Self-Loop) and the "Echo" (Reciprocity). These structures represent failures of the causal mechanism to propagate information forward.
...
Let $G_0$ denote the initial state. Then the existence of Directed Cycles of length $L \ge 3$ is excluded by the **Monotonicity of History** [(§1.3.4)](/monograph/rules/ontology/1.3/#1.3.4).
...
The **Monotonicity of History** [(§1.3.4)](/monograph/rules/ontology/1.3/#1.3.4) enforces strictly increasing timestamps along every directed path via the recurrence relation $H(e) = 1 + \max(H_{incoming})$. The application of the timestamp function $H$ to the edges of $C$ yields a chain of inequalities:
...
The inequality $x < x$ is false. The assumption of the existence of $C$ yields a logical contradiction. Furthermore, the existence of a cycle $L \ge 3$ implies pre-existing geometry, violating the constructive **Geometric Constructibility** [(§2.3.1)](/monograph/rules/axioms/2.3/#2.3.1).
...
Let $G_0$ denote the initial state. Then $G_0$ constitutes a Directed Acyclic Graph (DAG) **fundamental graph structures definition** [(§1.5.1)](/monograph/rules/ontology/1.5/#1.5.1), and the formation of any closed path is excluded as the strict monotonicity of the vertex depth function along all directed edges implies that the depth value strictly increases indefinitely within a finite set of integers.
...
Verification of the global causal consistency established in the **global acyclicity proof** [(§3.1.7.1)](/monograph/rules/architecture/3.1/#3.1.7.1) is based on the following protocols:
...
Let $G_0$ denote the initial state. Then $G_0$ constitutes a weakly connected graph, and disconnected configurations are excluded by **Acyclic Effective Causality** [(§2.7.1)](/monograph/rules/axioms/2.7/#2.7.1).
...
The effective influence order $\le$ decomposes into independent strict partial orders on each component. No directed path crosses component boundaries. The full relation $\le$ constitutes the disjoint union of the orders on the $C_i$. This decomposition is excluded by **Acyclic Effective Causality** [(§2.7.1)](/monograph/rules/axioms/2.7/#2.7.1).
...
Validation of the entropic penalty for disconnected topologies established in the **minimization of automorphisms proof** [(§3.1.8.1)](/monograph/rules/architecture/3.1/#3.1.8.1) is based on the following protocols:
...
Let $G$ denote a weakly connected DAG on $N$ vertices where the causal redundancy inherent to $|E| > N-1$ is excluded by the **Principle of Unique Causality** [(§2.3.3)](/monograph/rules/axioms/2.3/#2.3.3). Therefore, the vacuum state satisfies the exact sparsity condition $|E| = N-1$.
...
In the directed limit, an undirected cycle necessitates either multiple directed paths between a vertex pair or colliding causal flows. Both configurations constitute redundant information channels, which are excluded by the **Principle of Unique Causality** [(§2.3.3)](/monograph/rules/axioms/2.3/#2.3.3).
...
The **depth-parity bipartition lemma** [(§3.1.10)](/monograph/rules/architecture/3.1/#3.1.10) reveals a hidden symmetry in the vacuum: it is stratified. Because flow moves strictly away from the root, every step takes you exactly one level deeper into the causal history.
...
For all **graphs bipartite all**[(§1.5.1)](/monograph/rules/ontology/1.5/#1.5.1), odd-length cycles are topologically excluded. Therefore, the pre-existence of geometric quanta defined as **Directed 3-Cycles** [(§2.3.2)](/monograph/rules/axioms/2.3/#2.3.2) is excluded within the strictly bipartite vacuum state $G_0$ **the depth-parity bipartition lemma** [(§3.1.10)](/monograph/rules/architecture/3.1/#3.1.10).
...
The **Depth-Parity Bipartition** establishes the bipartition $(V_{\text{even}}, V_{\text{odd}})$ **the depth-parity bipartition lemma** [(§3.1.10)](/monograph/rules/architecture/3.1/#3.1.10). No edges exist within $V_{\text{even}}$ or within $V_{\text{odd}}$.
...
The **exclusion of odd cycles lemma** [(§3.1.11)](/monograph/rules/architecture/3.1/#3.1.11) constitutes the final nail in the coffin for pre-existing geometry.

* **Axiom $2$** defines the "Geometric Quantum" as a **$3$-cycle**.
* The number $3$ is **Odd**.
* The vacuum is **Bipartite** (the **depth-parity bipartition lemma** [(§3.1.10)](/monograph/rules/architecture/3.1/#3.1.10)).
...
:::tip[**Formal Derivation of the Finite Rooted Tree Topology via **Sequential Exclusion** [(§3.1.3)](/monograph/rules/architecture/3.1/#3.1.3)**]
:::
...
1.  **Infinite Graphs** [(§3.1.4)](/monograph/rules/architecture/3.1/#3.1.4): Filtered by **Well-Foundedness**, which strictly forbids infinite descending causal chains. $\Omega \to \Omega_{finite}$.
2.  **Cyclic Graphs** [(§3.1.7)](/monograph/rules/architecture/3.1/#3.1.7): Filtered by **Global Acyclicity**, which forbids the existence of closed directed loops based on depth monotonicity. $\Omega_{finite} \to \Omega_{DAG}$.
3.  **Disconnected Graphs** [(§3.1.8)](/monograph/rules/architecture/3.1/#3.1.8): Filtered by **Entropy Minimization** and the requirement for causal unity. $\Omega_{DAG} \to \Omega_{connected}$.
4.  **Dense Graphs** [(§3.1.9)](/monograph/rules/architecture/3.1/#3.1.9): Filtered by **Unique Causality**, which mandates $|E| = |V|-1$ to prevent redundant parallel paths. $\Omega_{connected} \to \Omega_{tree}$.
5.  **Non-Bipartite Graphs** [(§3.1.10)](/monograph/rules/architecture/3.1/#3.1.10): Filtered by **Depth Parity**, which mandates a strict partition $V_{even} \sqcup V_{odd}$. This structure topologically forbids odd-length cycles (the **exclusion of odd cycles lemma** [(§3.1.11)](/monograph/rules/architecture/3.1/#3.1.11)), establishing the pre-geometric state.
6.  **Bi-Directional Flows** [(§3.1.12)](/monograph/rules/architecture/3.1/#3.1.12): Filtered by **Asymmetry**, mandating a single source vertex (the Root) with strictly divergent flow.
