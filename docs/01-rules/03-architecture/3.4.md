...
1.  **The Instability Diagnosis** <Ref id="3.4.2" label="§3.4.2" />: The argument isolates topological tunneling, proving that the distance between the static, bipartite vacuum and an active, non-bipartite state is exactly one single-edge fluctuation.
2.  **The Nucleation Verification** <Ref id="3.4.3" label="§3.4.3" />: The argument proves that a symmetry-breaking edge fluctuation immediately constructs a compliant update site, converting the inert tree into an active domain.
3.  **The Catalytic Ignition** <Ref id="3.4.4" label="§3.4.4" />: The argument establishes that the execution of the first rewrite generates a stable directed three-cycle, creating the first geometric quantum that acts as a catalyst for subsequent updates.
4.  **The Inevitable Progression (the **ignition probability lemma** <Ref id="3.4.5" label="§3.4.5" />, the **demonstration of inevitable ignition proof** <Ref id="3.4.6" label="§3.4.6" />):** The argument derives a strictly positive lower bound for the transition probability, proving that the geometric phase transition completes on finite logical timescales.
...
1.  **Direct Edge $(u, w)$:** $E_0$ contains only even-odd edges. While the parities permit a connection, the tree structure of $G_0$ implies a unique path between any two nodes. A direct edge would create a triangle $(u, v, w)$, violating **Acyclicity** <Ref id="3.1.7" label="§3.1.7" />. Thus $(u, w) \notin E_0$.
2.  **Alternative 2-Path:** Any other path implies a cycle in the underlying undirected graph, violating the **Tree Condition** <Ref id="3.1.9" label="§3.1.9" />.
...
1.  **Input:** The compliant site $\pi = u \to v \to w$ established **previously** <Ref id="3.4.3" label="§3.4.3" />.
...
1.  **Topological Fragility** <Ref id="3.4.2" label="§3.4.2" />: It is established that the Hamming distance between the bipartite vacuum and a non-bipartite state is exactly $d_H = 1$ edge. The barrier to symmetry breaking is therefore not extensive but minimal.
2.  **Nucleation** <Ref id="3.4.3" label="§3.4.3" />: A single symmetry-breaking edge $e=(u,v)$ where $\pi(u)=\pi(v)$ creates a valid rewrite site by connecting vertices of identical parity. This bypasses the topological deadlock.
3.  **Supercriticality** <Ref id="3.4.4" label="§3.4.4" />: The formation of the first 3-cycle alters the local topology, creating new compliant 2-paths on its periphery. This triggers a branching ratio $b > 1$, leading to a runaway geometric cascade.
4.  **Thermodynamic Certainty** <Ref id="3.4.5" label="§3.4.5" />: In the pre-geometric limit where $T \to \infty$, the free energy barrier vanishes. The probability of a tunneling event per unit time is strictly positive ($P_{ign} > 0$).
