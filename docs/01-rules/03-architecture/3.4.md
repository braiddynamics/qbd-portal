...
The initial vacuum state $G_0$ constitutes a metastable **False Vacuum** characterized by strict bipartiteness [(§3.1.10)](/monograph/rules/architecture/3.1/#3.1.10), which topologically prohibits the formation of Geometric Quanta [(§2.3.2)](/monograph/rules/axioms/2.3/#2.3.2). It is asserted that a single non-perturbative **Tunneling Event** suffices to nucleate a seed that breaks the $\mathbb{Z}_2$ parity symmetry, generates the first compliant rewrite sites [(§3.3.2)](/monograph/rules/architecture/3.2/#3.2.2), and initiates a first-order phase transition to the geometric vacuum.
...
1.  **The Instability Diagnosis (Lemma [§3.4.2]):** The argument isolates topological tunneling, proving that the distance between the static, bipartite vacuum and an active, non-bipartite state is exactly one single-edge fluctuation.
2.  **The Nucleation Verification (Lemma [§3.4.3]):** The argument proves that a symmetry-breaking edge fluctuation immediately constructs a compliant update site, converting the inert tree into an active domain.
3.  **The Catalytic Ignition (Lemma [§3.4.4]):** The argument establishes that the execution of the first rewrite generates a stable directed three-cycle, creating the first geometric quantum that acts as a catalyst for subsequent updates.
4.  **The Inevitable Progression (Lemma [§3.4.5], Proof [§3.4.6]):** The argument derives a strictly positive lower bound for the transition probability, proving that the geometric phase transition completes on finite logical timescales.
...
1.  **Topological Fragility (Lemma [§3.4.2](/monograph/rules/architecture/3.4/#3.4.2)):** It is established that the Hamming distance between the bipartite vacuum and a non-bipartite state is exactly $d_H = 1$ edge. The barrier to symmetry breaking is therefore not extensive but minimal.
2.  **Nucleation (Lemma [§3.4.3](/monograph/rules/architecture/3.4/#3.4.3)):** A single symmetry-breaking edge $e=(u,v)$ where $\pi(u)=\pi(v)$ creates a valid rewrite site by connecting vertices of identical parity. This bypasses the topological deadlock.
3.  **Supercriticality (Lemma [§3.4.4](/monograph/rules/architecture/3.4/#3.4.4)):** The formation of the first 3-cycle alters the local topology, creating new compliant 2-paths on its periphery. This triggers a branching ratio $b > 1$, leading to a runaway geometric cascade.
4.  **Thermodynamic Certainty (Lemma [§3.4.5](/monograph/rules/architecture/3.4/#3.4.5)):** In the pre-geometric limit where $T \to \infty$, the free energy barrier vanishes. The probability of a tunneling event per unit time is strictly positive ($P_{ign} > 0$).
...
Quantification of the ignition robustness established in the High-T Probability Proof [(§3.4.5.1)](/monograph/rules/architecture/3.4/#3.4.5.1) is based on the following protocols:
