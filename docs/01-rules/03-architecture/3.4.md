...
The initial vacuum state $G_0$ constitutes a metastable **False Vacuum** characterized by **bipartiteness** [(§3.1.10)](/monograph/rules/architecture/3.1/#3.1.10), which topologically prohibits the formation of **Geometric Quanta** [(§2.3.2)](/monograph/rules/axioms/2.3/#2.3.2). It is asserted that a single non-perturbative **Tunneling Event** suffices to nucleate a seed that breaks the $\mathbb{Z}_2$ parity symmetry, generates the first compliant **rewrite sites** [(§3.3.2)](/monograph/rules/architecture/3.3/#3.3.2), and initiates a first-order phase transition to the geometric vacuum.
...
1.  **The Instability Diagnosis** [(§3.4.2)](/monograph/rules/architecture/3.4/#3.4.2): The argument isolates topological tunneling, proving that the distance between the static, bipartite vacuum and an active, non-bipartite state is exactly one single-edge fluctuation.
2.  **The Nucleation Verification** [(§3.4.3)](/monograph/rules/architecture/3.4/#3.4.3): The argument proves that a symmetry-breaking edge fluctuation immediately constructs a compliant update site, converting the inert tree into an active domain.
3.  **The Catalytic Ignition** [(§3.4.4)](/monograph/rules/architecture/3.4/#3.4.4): The argument establishes that the execution of the first rewrite generates a stable directed three-cycle, creating the first geometric quantum that acts as a catalyst for subsequent updates.
4.  **The Inevitable Progression (the **ignition probability lemma** [(§3.4.5)](/monograph/rules/architecture/3.4/#3.4.5), the **demonstration of inevitable ignition proof** [(§3.4.6)](/monograph/rules/architecture/3.4/#3.4.6)):** The argument derives a strictly positive lower bound for the transition probability, proving that the geometric phase transition completes on finite logical timescales.
...
Let $G_0 = (V, E_0)$ denote the vacuum state. The **Depth-Parity Bipartition** [(§3.1.10)](/monograph/rules/architecture/3.1/#3.1.10) establishes that $G_0$ admits a canonical 2-coloring:
...
The **Elementary Task Space** [(§1.4.1)](/monograph/rules/ontology/1.4/#1.4.1) permits single-edge additions: thus, the transition barrier is kinematic rather than combinatorial.
...
The removal of $e_{\text{tunnel}}$ would require a specific inverse operation. However, the **Strict Timestamps** **requirement** [(§2.6.3)](/monograph/rules/axioms/2.6/#2.6.3) prohibits the deletion of edges once established in the causal order (except via specific rewrite rules which do not apply to isolated edges). Therefore, the symmetry breaking is persistent:
...
For any Tunneling Event $e=(u, v)$ in $G_0$ and vertex $w$ such that $(v, w) \in E_0$, the directed path $(u, v, w)$ constitutes a compliant 2-**Path** [(§1.5.2)](/monograph/rules/ontology/1.5/#1.5.2). In particular, this path satisfies the **Principle of Unique Causality** [(§2.3.3)](/monograph/rules/axioms/2.3/#2.3.3) and constitutes a valid input for the rewrite rule.
...
Let $G_1$ denote the state immediately following the tunneling event $e_{\text{tunnel}} = (u, v)$ where $u, v \in V_{\text{even}}$. The underlying structure of $G_0$ constitutes a **Maximally Branched Tree** [(§3.2.6)](/monograph/rules/architecture/3.2/#3.2.6). Consequently, the internal vertex $v$ possesses an out-degree $k \ge 1$:
...
The **Principle of Unique Causality** [(§2.3.3)](/monograph/rules/axioms/2.3/#2.3.3) imposes the requirement that no other path of length $\le 2$ exists between $u$ and $w$.

1.  **Direct Edge $(u, w)$:** $E_0$ contains only even-odd edges. While the parities permit a connection, the tree structure of $G_0$ implies a unique path between any two nodes. A direct edge would create a triangle $(u, v, w)$, violating **Acyclicity** [(§3.1.7)](/monograph/rules/architecture/3.1/#3.1.7). Thus $(u, w) \notin E_0$.
2.  **Alternative 2-Path:** Any other path implies a cycle in the underlying undirected graph, violating the **Tree Condition** [(§3.1.9)](/monograph/rules/architecture/3.1/#3.1.9).
...
Let the rewrite rule $\mathcal{R}$ be applied to the tunneling-induced compliant 2-Path $(u, v, w)$. Then the operation generates the closing edge $(w, u)$, forming the first **Directed 3-Cycle** [(§2.3.2)](/monograph/rules/axioms/2.3/#2.3.2) in the universe, constituting the initial quantum of spatial area and acting as a catalytic seed for subsequent geometric growth.
...
1.  **Input:** The compliant site $\pi = u \to v \to w$ established **previously** [(§3.4.3)](/monograph/rules/architecture/3.4/#3.4.3).
...
:::tip[the **Formal Derivation of the Deterministic Transition to Geometry via Thermodynamic Probability** [(§3.4.1)](/monograph/rules/architecture/3.4/#3.4.1)]
:::
...
1.  **Topological Fragility** [(§3.4.2)](/monograph/rules/architecture/3.4/#3.4.2): It is established that the Hamming distance between the bipartite vacuum and a non-bipartite state is exactly $d_H = 1$ edge. The barrier to symmetry breaking is therefore not extensive but minimal.
2.  **Nucleation** [(§3.4.3)](/monograph/rules/architecture/3.4/#3.4.3): A single symmetry-breaking edge $e=(u,v)$ where $\pi(u)=\pi(v)$ creates a valid rewrite site by connecting vertices of identical parity. This bypasses the topological deadlock.
3.  **Supercriticality** [(§3.4.4)](/monograph/rules/architecture/3.4/#3.4.4): The formation of the first 3-cycle alters the local topology, creating new compliant 2-paths on its periphery. This triggers a branching ratio $b > 1$, leading to a runaway geometric cascade.
4.  **Thermodynamic Certainty** [(§3.4.5)](/monograph/rules/architecture/3.4/#3.4.5): In the pre-geometric limit where $T \to \infty$, the free energy barrier vanishes. The probability of a tunneling event per unit time is strictly positive ($P_{ign} > 0$).
...
Quantification of the ignition robustness established in the **ignition probability proof** [(§3.4.5.1)](/monograph/rules/architecture/3.4/#3.4.5.1) is based on the following protocols:
