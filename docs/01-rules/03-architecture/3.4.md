...
2.  **Symmetry Breaking Nucleation** <Ref id="3.4.3" label="§3.4.3" />: The lemma proves that a symmetry-breaking edge fluctuation immediately constructs a compliant update site, converting the inert tree into an active domain.
3.  **Geometric Quantum Generation** <Ref id="3.4.4" label="§3.4.4" />: The argument establishes that the execution of the first rewrite generates a stable directed three-cycle, creating the first geometric quantum that acts as a catalyst for subsequent updates.
4.  **Ignition Probability Bounds** <Ref id="3.4.5" label="§3.4.5" />: The lemma derives a strictly positive lower bound for the transition probability, proving that the geometric phase transition completes on finite logical timescales.
5.  **Inevitable Ignition Derivation** <Ref id="3.4.6" label="§3.4.6" />: The proof combines these selective filters and probability bounds to demonstrate the deterministic transition from the pre-geometric vacuum to the geometric phase.
