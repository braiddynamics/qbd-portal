...
1. **The Horizon Problem** [(§2.7.3)](/monograph/rules/axioms/2.7/#2.7.3): The argument establishes that graph evolution generates system-spanning cycles that exceed any finite computational radius, proving that local operators remain topologically blind to global loop closure.
2. **The Local Approximation** [(§2.7.4)](/monograph/rules/axioms/2.7/#2.7.4): The argument demonstrates that a local check of logarithmic radius is mathematically sufficient, proving that the probability of a cycle evading detection decays exponentially with the check radius.
3. **The Energetic Divergence** [(§2.7.5)](/monograph/rules/axioms/2.7/#2.7.5): The argument proves that post-hoc deletion of global cycles requires signal synchronization across the graph diameter, demonstrating that the required energy diverges for infinite networks and establishing the necessity of preemptive local enforcement.
...
The **local puc approximation lemma** [(§2.7.4)](/monograph/rules/axioms/2.7/#2.7.4) introduces a crucial philosophical and physical nuance: the enforcement of Axiom $3$ is **probabilistic** (not absolute) in the limit of infinite size. However, the probability of error is exponentially suppressed, which aligns this theory with the foundations of statistical mechanics as formalized by **[(van Kampen, 1992)](/monograph/appendices/a-references#A.63)**. In his treatment of stochastic processes, van Kampen demonstrates how macroscopic deterministic laws (like the diffusion equation) emerge from microscopic probabilistic jumps (the master equation) simply through the law of large numbers.
...
:::tip[the **Formal Proof of Thermodynamic Enforcement** [(§2.7.2)](/monograph/rules/axioms/2.7/#2.7.2) via Demonstration of Energy Divergence]
:::
...
By the **cycle diameter growth lemma** [(§2.7.3)](/monograph/rules/axioms/2.7/#2.7.3), the loop length $L$ exceeds the local horizon $R$.
...
The **independence of axiom 3 theorem** [(§2.7.6)](/monograph/rules/axioms/2.7/#2.7.6) serves as the capstone of the axiomatic chapter, confirming that the theory requires a "Tripartite" foundation where no single pillar is redundant. We may view these axioms as the three legs of a stool upon which physical reality rests.
