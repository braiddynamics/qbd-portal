...
1. **The Horizon Problem (Lemma [§2.7.3]):** The argument establishes that graph evolution generates system-spanning cycles that exceed any finite computational radius, proving that local operators remain topologically blind to global loop closure.
2. **The Local Approximation (Lemma [§2.7.4]):** The argument demonstrates that a local check of logarithmic radius is mathematically sufficient, proving that the probability of a cycle evading detection decays exponentially with the check radius.
3. **The Energetic Divergence (Proof [§2.7.5]):** The argument proves that post-hoc deletion of global cycles requires signal synchronization across the graph diameter, demonstrating that the required energy diverges for infinite networks and establishing the necessity of preemptive local enforcement.
