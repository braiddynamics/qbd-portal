...
3.  **Sequentiality:** The creation timestamps of the constituent edges are strictly increasing, such that $H(v_i, v_{i+1}) < H(v_{i+1}, v_{i+2})$ for all valid $i$, preserving the **ordering historical** [(§1.3.4)](/monograph/rules/ontology/1.3/#1.3.4).

**Technical Note on Cycle Traversal:** The **effective influence definition** [(§2.6.1)](/monograph/rules/axioms/2.6/#2.6.1) of $\le$ requires $\pi_{uv}$ to be a *simple* directed path. Consequently, a vertex sequence containing repeated nodes does not constitute a valid trajectory for the purposes of establishing effective influence.
...
1. **The Local Trap** [(§2.6.3)](/monograph/rules/axioms/2.6/#2.6.3): The argument demonstrates that strictly increasing local timestamps fail to detect circularity that closes beyond the causal horizon of a single vertex.
2. **The Reflexive Loop** [(§2.6.4)](/monograph/rules/axioms/2.6/#2.6.4): The argument dissects the closed three-cycle under local rules, proving that transitively closed paths yield a reflexive self-influence that violates causal irreflexivity.
3. **The Symmetric Loop** [(§2.6.5)](/monograph/rules/axioms/2.6/#2.6.5): The argument constructs a symmetric bowtie configuration, proving that disjoint sub-paths permit mutual influence between distinct vertices despite strictly monotonic local updates.
4. **The Global Necessity** [(§2.6.6)](/monograph/rules/axioms/2.6/#2.6.6): The argument synthesizes these local failures, demonstrating that local primitives license global causal closures and establishing the logical necessity of a global partial ordering constraint.
...
Let $v$ denote a vertex participating in a Geometric Quantum (Directed $3$-Cycle) with strictly increasing timestamps along the edges. Then the Effective Influence relation satisfies the reflexive condition $v \le v$, violating the global **requirement acyclicity global**[(§2.7.1)](/monograph/rules/axioms/2.7/#2.7.1).
...
1.  **Reflexivity Failure** [(§2.6.4)](/monograph/rules/axioms/2.6/#2.6.4): Within the local geometry of the $3$-cycle, the combination of directed edges and strictly increasing timestamps necessitates that upon closure of the loop, the relation $v \le v$ is established. This constitutes a violation of **Global Irreflexivity**.
2.  **Asymmetry Failure** [(§2.6.5)](/monograph/rules/axioms/2.6/#2.6.5): Within a $4$-cycle "Bowtie" configuration, the existence of disjoint sub-paths allows for the simultaneous establishment of $u \le v$ and $v \le u$ with valid timestamps. This constitutes a violation of **Global Asymmetry**.
