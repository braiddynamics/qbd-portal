...
1. **The Local Trap (Lemma [§2.6.3]):** The argument demonstrates that strictly increasing local timestamps fail to detect circularity that closes beyond the causal horizon of a single vertex.
2. **The Reflexive Loop (Lemma [§2.6.4]):** The argument dissects the closed three-cycle under local rules, proving that transitively closed paths yield a reflexive self-influence that violates causal irreflexivity.
3. **The Symmetric Loop (Lemma [§2.6.5]):** The argument constructs a symmetric bowtie configuration, proving that disjoint sub-paths permit mutual influence between distinct vertices despite strictly monotonic local updates.
4. **The Global Necessity (Proof [§2.6.6]):** The argument synthesizes these local failures, demonstrating that local primitives license global causal closures and establishing the logical necessity of a global partial ordering constraint.
