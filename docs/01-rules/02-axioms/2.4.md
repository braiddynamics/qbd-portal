...
1. **The Deadlock Avoidance** [(§2.4.2)](/monograph/rules/axioms/2.4/#2.4.2): The argument establishes confluence for local rewrites, proving that overlapping repairs do not block one another and ensuring that the reduction process avoids frozen states.
2. **The Target Existence** [(§2.4.3)](/monograph/rules/axioms/2.4/#2.4.3): The argument establishes chordlessness, proving that any maximal cycle maintains a hollow topology that exposes its internal vertices to triangulation.
3. **The Topological Ratchet** [(§2.4.4)](/monograph/rules/axioms/2.4/#2.4.4): The argument proves that the deletion mechanism operates strictly monotonically, preventing the system from reconstructing higher-complexity cycles once reduced.
4. **The Synthesis** [(§2.4.6)](/monograph/rules/axioms/2.4/#2.4.6): The argument combines the confluence, chordlessness, and monotonicity guarantees, proving that any state containing a cycle of length four or greater admits a valid sequence of transitions that terminates at a stable quantum geometry.
...
Let $e$ denote an edge belonging to a simple cycle $C$ of maximal length within a graph $G$ characterized by the Lexicographic Potential $\Phi(G)$ **lexicographic potential definition** [(§2.3.4)](/monograph/rules/axioms/2.3/#2.3.4). Then the deletion of $e$ yields a graph $G'$ satisfying the strict descent condition $\Phi(G') < \Phi(G)$.
...
1.  **Site Availability:** Maximal cycles constitute **structures chordless constitute**[(§2.4.3)](/monograph/rules/axioms/2.4/#2.4.3), ensuring the existence of valid 2-paths.
2.  **Structure Decomposition:** The addition of chords partitions maximal cycles into 3-cycles and smaller loops.
3.  **Cycle Bounding:** The **Principle of Unique Causality (PUC)** restricts additions to sites lacking **paths short lacking**[(§2.3.3)](/monograph/rules/axioms/2.3/#2.3.3). The creation of a cycle $L_{new} > L_{\max}$ requires a pre-existing path of length $> L_{\max}-1$ connecting vertices at distance 2. This implies a prior path violation.
...
2.  **Potential Drop:** Edge removal strictly reduces the **potential lexicographic** [(§2.4.4)](/monograph/rules/axioms/2.4/#2.4.4).
    $$
...
:::tip[**Formal Proof of the Decomposition the **general cycle decomposition theorem** [(§2.4.1)](/monograph/rules/axioms/2.4/#2.4.1) via Synthesis of Confluence and Potential Reduction**]
:::
...
1.  **Site Existence:** Cycles of length $L$ possess **topology chordless possess**[(§2.4.3)](/monograph/rules/axioms/2.4/#2.4.3). This guarantees the presence of compliant 2-paths susceptible to the rewrite rule $\mathcal{R}$.
...
1.  **Confluence:** The parallel application of operations proceeds **conflict without proceeds**[(§2.4.2)](/monograph/rules/axioms/2.4/#2.4.2), yielding state $G_{add}$.
2.  **Net Descent:** The subsequent deletion phase produces state $G_1$ satisfying $\Phi(G_1) < \Phi(G_0)$ **decrease in parallel updates lemma** [(§2.4.5)](/monograph/rules/axioms/2.4/#2.4.5).

**IV. Iterative Termination**

1.  **Sequence Construction:** The dynamics generate a sequence of potentials $\Phi(G_0) > \Phi(G_1) > \dots$.
2.  **Well-Foundedness:** The lexicographic order on finite graphs contains no infinite **chains descending infinite**[(§2.3.4)](/monograph/rules/axioms/2.3/#2.3.4).
...
Confirmation of the finite termination condition established in the General Cycle Decomposition the **general cycle decomposition proof** [(§2.4.6)](/monograph/rules/axioms/2.4/#2.4.6) is based on the following protocols:
