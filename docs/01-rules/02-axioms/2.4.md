...
1. **The Deadlock Avoidance** <Ref id="2.4.2" label="§2.4.2" />: The argument establishes confluence for local rewrites, proving that overlapping repairs do not block one another and ensuring that the reduction process avoids frozen states.
2. **The Target Existence** <Ref id="2.4.3" label="§2.4.3" />: The argument establishes chordlessness, proving that any maximal cycle maintains a hollow topology that exposes its internal vertices to triangulation.
3. **The Topological Ratchet** <Ref id="2.4.4" label="§2.4.4" />: The argument proves that the deletion mechanism operates strictly monotonically, preventing the system from reconstructing higher-complexity cycles once reduced.
4. **The Synthesis** <Ref id="2.4.6" label="§2.4.6" />: The argument combines the confluence, chordlessness, and monotonicity guarantees, proving that any state containing a cycle of length four or greater admits a valid sequence of transitions that terminates at a stable quantum geometry.
...
1.  **Site Availability:** Maximal cycles constitute **structures chordless constitute<Ref id="2.4.3" label="§2.4.3" />, ensuring the existence of valid 2-paths.
2.  **Structure Decomposition:** The addition of chords partitions maximal cycles into 3-cycles and smaller loops.
3.  **Cycle Bounding:** The **Principle of Unique Causality (PUC)** restricts additions to sites lacking **paths short lacking<Ref id="2.3.3" label="§2.3.3" />. The creation of a cycle $L_{new} > L_{\max}$ requires a pre-existing path of length $> L_{\max}-1$ connecting vertices at distance 2. This implies a prior path violation.
...
2.  **Potential Drop:** Edge removal strictly reduces the **potential lexicographic** <Ref id="2.4.4" label="§2.4.4" />.
    $$
...
2.  **Net Descent:** The subsequent deletion phase produces state $G_1$ satisfying $\Phi(G_1) < \Phi(G_0)$ **decrease in parallel updates lemma** <Ref id="2.4.5" label="§2.4.5" />.

**IV. Iterative Termination**

1.  **Sequence Construction:** The dynamics generate a sequence of potentials $\Phi(G_0) > \Phi(G_1) > \dots$.
2.  **Well-Foundedness:** The lexicographic order on finite graphs contains no infinite **descending chains** <Ref id="2.3.4" label="§2.3.4" />.
...
### 2.4.11 Commentary: Arrow of Simplicity {#2.4.11}
