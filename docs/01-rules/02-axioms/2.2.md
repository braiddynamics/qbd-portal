...
1. **The Topological Identification** [(§2.2.2)](/monograph/rules/axioms/2.2/#2.2.2): The argument establishes that a reflexive edge is topologically identical to a directed cycle of length one, proving that any relation permitting reflexivity violates acyclicity.
2. **The Thermodynamic Nullity** [(§2.2.3)](/monograph/rules/axioms/2.2/#2.2.3): The argument quantifies the information content of reflexive edges, demonstrating that a self-loop contributes exactly zero to the path entropy, which renders the structure physically inert and incapable of recording history.
3. **The Counter-Model** [(§2.2.4)](/monograph/rules/axioms/2.2/#2.2.4): The argument constructs a formal model that satisfies mathematical antisymmetry yet fails causal validity, establishing the absolute necessity of irreflexivity.
...
Let $e = (u, u)$ denote a self-loop incident to a vertex $u$. Then this structure constitutes a directed cycle of length $k=1$ **cycle definitions** [(§1.5.3)](/monograph/rules/ontology/1.5/#1.5.3), a configuration excluded by **Directed Acyclic Graph** [(§1.5.1)](/monograph/rules/ontology/1.5/#1.5.1).
...
Let a directed cycle of length $k$ be defined as a sequence of vertices $C_k = (v_0, v_1, \dots, v_k)$ satisfying **conditions two** [(§1.5.3)](/monograph/rules/ontology/1.5/#1.5.3):
...
The self-loop $e_{loop}$ satisfies the definition of a directed cycle $C_1$. We conclude that the existence of such an edge violates the acyclicity condition required for a valid **history causal valid**[(§1.5.1)](/monograph/rules/ontology/1.5/#1.5.1).
...
1.  **Topological Pathology** [(§2.2.2)](/monograph/rules/axioms/2.2/#2.2.2): It is established that a reflexive edge $e = (u, u)$ constitutes a directed cycle of length $k=1$. The existence of such a structure stands in direct violation of the **Global Acyclicity** requirement, which is essential for defining a valid causal history.
2.  **Thermodynamic Nullity** [(§2.2.3)](/monograph/rules/axioms/2.2/#2.2.3): It is established that the addition of a self-loop yields a net entropic gain of exactly zero ($\Delta S = 0$). This occurs because the relation fails to distinguish the vertex from itself or establish a correlation between distinct entities. The operation consumes a unit of logical time $t_L$ without generating distinguishable information, thereby violating the requirement for effective physical evolution.
