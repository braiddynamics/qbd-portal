...
1.  **Clause A (Positive Construction):** The formation of closed topological structures is restricted exclusively to **Geometric Quanta**, defined as **3-Cycle** <Ref id="1.5.8" label="§1.5.8" />. The closure of a causal loop is permissible if and only if the resulting path sequence has a length of exactly $L=3$.
...
│
├── 2.3.3 Principle of Unique Causality (PUC)
