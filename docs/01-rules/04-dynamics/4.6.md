...
2.  **Probabilistic Rewrite ($\mathcal{R}^\flat$):** The monadic extension of the Universal Constructor $\mathcal{R}$ [(§4.5.1)](/monograph/rules/dynamics/4.5/#4.5.1), acting as a transition kernel to generate a provisional measure $\mu_{prov}$ over potential successors.
...
Quantification of the information loss inherent in the Time Evolution Operator $\mathcal{U}$ established in the Irreversibility Theorem [(§4.6.3)](/monograph/rules/dynamics/4.6/#4.6.3) is based on the following protocols:
