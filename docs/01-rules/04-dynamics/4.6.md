...
The **Evolution Operator**, denoted $\mathcal{U}$, is defined as a stochastic endomorphism acting upon the state space of valid causal graphs. Let $\Sigma_{\text{valid}}$ be the set of all axiomatically **graphs compliant axiomatically**[(§1.3.1)](/monograph/rules/ontology/1.3/#1.3.1) and $\mathcal{P}(\Sigma_{\text{valid}})$ be the space of probability measures over this set. The operator $\mathcal{U}: \mathcal{P}(\Sigma_{\text{valid}}) \to \mathcal{P}(\Sigma_{\text{valid}})$ is constructed as the sequential composition of four distinct maps:
...
1.  **Awareness Lift ($\mathcal{P}(R_T)$):** The functorial lift of the Awareness Endofunctor $R_T$ **the awareness endofunctor ($r_t$) definition** [(§4.3.2)](/monograph/rules/dynamics/4.3/#4.3.2), mapping the measure space to the annotated domain $\mathcal{P}(\mathbf{AnnCG})$.
2.  **Probabilistic Rewrite ($\mathcal{R}^\flat$):** The monadic extension of the Universal Constructor $\mathcal{R}$ **the universal constructor definition** [(§4.5.1)](/monograph/rules/dynamics/4.5/#4.5.1), acting as a transition kernel to generate a provisional measure $\mu_{prov}$ over potential successors.
