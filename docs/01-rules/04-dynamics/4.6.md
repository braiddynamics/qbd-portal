...
The **Evolution Operator**, denoted $\mathcal{U}$, is defined as a stochastic endomorphism acting upon the state space of valid causal graphs. Let $\Sigma_{\text{valid}}$ be the set of all **axiomatically compliant graphs** [(§1.3.1)](/monograph/rules/ontology/1.3/#1.3.1) and $\mathcal{P}(\Sigma_{\text{valid}})$ be the space of probability measures over this set. The operator $\mathcal{U}: \mathcal{P}(\Sigma_{\text{valid}}) \to \mathcal{P}(\Sigma_{\text{valid}})$ is constructed as the sequential composition of four distinct maps:
...
1.  **Awareness Lift ($\mathcal{P}(R_T)$):** The functorial lift of the Awareness Endofunctor $R_T$ [(§4.3.2)](/monograph/rules/dynamics/4.3/#4.3.2), mapping the measure space to the annotated domain $\mathcal{P}(\mathbf{AnnCG})$.
2.  **Probabilistic Rewrite ($\mathcal{R}^\flat$):** The monadic extension of the Universal Constructor $\mathcal{R}$ **universal constructor** [(§4.5.1)](/monograph/rules/dynamics/4.5/#4.5.1), acting as a transition kernel to generate a provisional measure $\mu_{prov}$ over potential successors.
3.  **Measurement Projection ($\mathcal{M}$):** The non-linear projection map that annihilates support on states violating the **Hard Constraint Projectors** [(§3.5.4)](/monograph/rules/architecture/3.5/#3.5.4) and re-normalizes the remaining measure.
...
1.  **Additions:** The base rate $\mathbb{P}_{\text{add}} = 1$ follows from **The Addition Probability** [(§4.5.4)](/monograph/rules/dynamics/4.5/#4.5.4).
2.  **Deletions:** The base rate $\mathbb{P}_{\text{del}} = 1/2$ follows from **The Deletion Probability** [(§4.5.6)](/monograph/rules/dynamics/4.5/#4.5.6).

**III. Modulation Factor**

Each event $u_i$ is modulated by $\chi_i(\sigma)$, the local **Catalytic Tension Factor** [(§4.5.2)](/monograph/rules/dynamics/4.5/#4.5.2):
...
Verification of the emergent probability weights established in the Born Rule Proof [(§4.6.2.1)](/monograph/rules/dynamics/4.6/#4.6.2.1) is based on the following protocols:
...
The simulation confirms that the normalized probability of the single-addition path is $0.400$, while the mixed path (two additions + one deletion) is $0.200$. The ratio $P_C / P_A = 0.50$ confirms that the deletion event introduces an exact penalty factor of $1/2$. This validates the **born rule theorem** [(§4.6.2)](/monograph/rules/dynamics/4.6/#4.6.2) that transition probabilities follow the product rule of their constituent micro-events, reproducing the Born Rule structure from pure counting statistics.
...
This result provides a startlingly classical mechanism for the emergence of Born-like probabilities. The scaling factor $(1/2)^{N_{\text{del}}}$ does not arise from a complex wave equation or Hilbert space norm, but from the naked entropic "cost" of information erasure. This derivation suggests a physical origin for the principles of **[(Zurek, 2003)](/monograph/appendices/a-references#A.73)**, where quantum probabilities (the Born rule) emerge from the symmetries of entanglement and the environment's selection of stable states. In QBD, the "environment" is the vacuum friction that selects against information loss.
...
### 4.6.3.1 Proof: The Thermodynamic Arrow {#4.6.3.1}
...
Quantification of the information loss inherent in the Time Evolution Operator $\mathcal{U}$ established in the Thermodynamic Arrow Proof [(§4.6.3.1)](/monograph/rules/dynamics/4.6/#4.6.3.1) is based on the following protocols:
