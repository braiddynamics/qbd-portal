...
This implementation adheres to the Micro/Macro separation principle, operating exclusively on local variables with universal constants derived in the **thermodynamic foundations section** [(§4.4)](/monograph/rules/dynamics/4.4/#4.4).
...
1. **The Generative Drive (the **addition mode definition** [(§4.5.3)](/monograph/rules/dynamics/4.5/#4.5.3), the **the addition probability theorem** [(§4.5.4)](/monograph/rules/dynamics/4.5/#4.5.4)):** The constructor identifies compliant two-paths and computes addition probabilities, driving the generation of new geometric connectivity in sparse regions.
2. **The Pruning Balance (the **deletion mode definition** [(§4.5.5)](/monograph/rules/dynamics/4.5/#4.5.5), the **the deletion probability theorem** [(§4.5.6)](/monograph/rules/dynamics/4.5/#4.5.6)):** The constructor scans for existing three-cycles and computes deletion probabilities, balancing growth with stress-driven pruning.
3. **The Adaptive Feedback** [(§4.5.2)](/monograph/rules/dynamics/4.5/#4.5.2): The constructor modulates rewrite rates via the catalytic tension factor, establishing a self-regulating dynamical feedback loop that stabilizes the system.
