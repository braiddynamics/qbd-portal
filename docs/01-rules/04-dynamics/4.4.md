...
Let $\mu$ denote the **Friction Coefficient**. Then $\mu$ constitutes the normalization constant $\mu = \frac{1}{\sqrt{2\pi}} \approx 0.399$. Moreover, this value forms the Gaussian normalization required by **Geometric Autocatalysis ($J_{auto}$)** <Ref id="5.2.4" label="§5.2.4" />.
