...
For every morphism $p: u \to v$ in $\mathbf{Caus}_t$, the composition with the Trivial Path [(§4.1.1)](/monograph/rules/dynamics/4.1/#4.1.1) satisfies the identity laws $p \circ \text{id}_u = p$ and $\text{id}_v \circ p = p$. The concatenation of a sequence with a zero-length sequence yields the original sequence invariant
...
The composition of History-Respecting Embeddings [(§4.1.3)](/monograph/rules/dynamics/4.1/#4.1.3) preserves the monotonicity of timestamps. If $f: G \to G'$ and $g: G' \to G''$ are valid morphisms in $\mathbf{Hist}$, then for any edge $e \in G$, the inequality chain $H(e) \leq H'(f(e)) \leq H''(g(f(e)))$ holds. Transitivity of the definition ensures that $g \circ f$ is a valid morphism in $\mathbf{Hist}$.
...
The **Effective Influence** relation $\le$ [(§2.6.1)](/monograph/rules/axioms/2.6/#2.6.1) is formally encoded as a constrained subset of morphisms within $\mathbf{Caus}_t$. Specifically, $u \le v$ holds if and only if there exists a morphism $p \in \text{Hom}(u, v)$ such that the path length is $\ell \ge 2$ and the sequence of edge timestamps is strictly increasing.
...
1.  **Irreflexivity:** No morphism with $\ell \ge 2$ and strictly increasing timestamps can map $u$ to $u$ without violating Acyclicity [(§2.7.1)](/monograph/rules/axioms/2.7/#2.7.1).
...
**Acyclic Effective Causality** [(§2.7.1)](/monograph/rules/axioms/2.7/#2.7.1) forbids all cycles.
