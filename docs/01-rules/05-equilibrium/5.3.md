...
3.  **Ignition Injection:** A symmetry-breaking edge $(u, v)$ is added to the ZPI vacuum such that $\pi(u) = \pi(v)$ **inevitable geometrogenesis theorem** [(§3.4.1)](/monograph/rules/architecture/3.4/#3.4.1), creating the first 3-Cycle ($H=1$) and transforming the inert vacuum into an active initial state.
4.  **Evolution and Aggregation:** The system is advanced via 1500 iterative applications of the Evolution Operator $\mathcal{U}$ **the evolution operator definition** [(§4.6.1)](/monograph/rules/dynamics/4.6/#4.6.1). Observables (specifically $N_3$ and $\rho_3$) are recorded at each tick, and statistical moments (mean, median, skew) are aggregated across the ensemble.
...
Verification of the phase space trajectories established in the Master Equation Proof [(§5.2.7)](/monograph/rules/equilibrium/5.2/#5.2.7) is based on the following protocols:
...
### 5.3.3.3 Diagram: The Vacuum Viability Heat Map
