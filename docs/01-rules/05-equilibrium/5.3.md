...
3.  **Ignition Injection:** A symmetry-breaking edge $(u, v)$ is added to the ZPI vacuum such that $\pi(u) = \pi(v)$ **inevitable geometrogenesis theorem** <Ref id="3.4.1" label="§3.4.1" />, creating the first 3-Cycle ($H=1$) and transforming the inert vacuum into an active initial state.
4.  **Evolution and Aggregation:** The system is advanced via 1500 iterative applications of the Evolution Operator $\mathcal{U}$ **the evolution operator definition** <Ref id="4.6.1" label="§4.6.1" />. Observables (specifically $N_3$ and $\rho_3$) are recorded at each tick, and statistical moments (mean, median, skew) are aggregated across the ensemble.
...
Verification of the phase space trajectories established in the Master Equation Proof <Ref id="5.2.7" label="§5.2.7" /> is based on the following protocols:
