...
In the dilute limit ($\rho \to 0$), cycles are isolated. The deletion of a geometric quantum is a spontaneous symmetry-breaking event governed by the Boltzmann probability at the critical temperature $T_c$. As established in the **the deletion probability theorem** [(§4.5.6)](/monograph/rules/dynamics/4.5/#4.5.6), the base deletion probability per cycle is $\mathbb{P}_0 = 1/2$.
$$
