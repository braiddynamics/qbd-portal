...
Verification of the combinatorial derivation established in the Combinatorial Precursors Proof <Ref id="5.2.4.1" label="§5.2.4.1" /> is based on the following protocols:
...
Validation of the exponential suppression factor established in the Friction Term Derivation Proof <Ref id="5.2.5.1" label="§5.2.5.1" /> is based on the following protocols:
...
Validation of the catalytic stress term established in the Instability Derivation Proof <Ref id="5.2.6.1" label="§5.2.6.1" /> is based on the following protocols:
...
1. **Vacuum Permittivity:** The **Vacuum Permittivity** <Ref id="5.2.3" label="§5.2.3" /> establishes the baseline spontaneous loop closure probability at zero geometric density, yielding the background flux constant $\Lambda \approx 0.0156$.
2. **Geometric Autocatalysis:** The **Geometric Autocatalysis** <Ref id="5.2.4" label="§5.2.4" /> quantifies the induced loop creation flux scaling with the density of open 2-paths, establishing the non-linear growth component $9\rho^2$.
3. **Frictional Suppression:** The **Frictional Suppression** <Ref id="5.2.5" label="§5.2.5" /> enforces the exponential damping of update acceptance rates due to steric hindrance, imposing the suppression factor $e^{-6\mu\rho}$ where $\mu = \frac{1}{\sqrt{2\pi}}$.
4. **Entropic & Catalytic Decay:** The **Entropic & Catalytic Decay** <Ref id="5.2.6" label="§5.2.6" /> aggregates the spontaneous informational evaporation and quadratic catalytic stress terms, establishing the total deletion current $\frac{1}{2}\rho(1 + 6\lambda_{\text{cat}}\rho)$ where $\lambda_{\text{cat}} = e - 1$.
