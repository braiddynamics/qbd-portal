...
* **$\Lambda$:** The Vacuum Drive, which is the baseline osmotic pressure of the graph substrate [(§5.2.3)](/monograph/rules/equilibrium/5.2/#5.2.3).
* **$9\rho^2$:** The combinatorial density of compliant 2-path precursors (Autocatalysis) [(§5.2.4)](/monograph/rules/equilibrium/5.2/#5.2.4).
* **$e^{-6\mu\rho}$:** The frictional suppression factor arising from Acyclic constraints [(§5.2.5)](/monograph/rules/equilibrium/5.2/#5.2.5).
...
1. **The Vacuum Permittivity (Lemma [§5.2.3]):** The argument isolates the spontaneous edge creation rate of the vacuum tree, establishing a non-zero base probability that ignites the system from a null geometric state.
2. **The Autocatalytic Growth (Lemma [§5.2.4]):** The argument quantifies the non-linear growth dynamics scaling quadratically with existing spatial density, representing the macroscopic driver of the geometric phase transition.
3. **The Frictional Suppression (Lemma [§5.2.5]):** The argument derives the exponential friction governor that dampens edge creation as density rises, modeling the increased likelihood of causal loop violations.
4. **The Net Flux Synthesis (Lemma [§5.2.6], Proof [§5.2.7]):** The argument aggregates creation and stress-catalyzed deletion rates, yielding the intensive density master equation that stabilizes at a finite spatial equilibrium.
...
The creation flux is composed of two distinct driving forces: the constant background vacuum permittivity $\Lambda$ (Lemma [§5.2.3](/monograph/rules/equilibrium/5.2/#5.2.3)) and the quadratic autocatalytic growth $9\rho^2$ (Lemma [§5.2.4](/monograph/rules/equilibrium/5.2/#5.2.4)). This total potential is modulated by the geometric probability of satisfying the topological constraints, which imposes the Gaussian friction factor $e^{-6\mu\rho}$ derived from the stress distribution (Theorem [§4.4.6](/monograph/rules/dynamics/4.4/#4.4.6)).
$$
J_{in} = (\Lambda + 9\rho^2) e^{-6\mu\rho}, \quad \text{where } \mu = \frac{1}{\sqrt{2\pi}}
$$

**III. Total Deletion Potential ($J_{out}$)**
The deletion flux is governed by the thermodynamic probability of information erasure. It consists of the linear entropic decay of independent cycles, $\frac{1}{2}\rho$, and the non-linear catalytic stress term, $3\lambda_{cat}\rho^2$. The coefficient $\lambda_{cat}$ is determined by the entropic release of tension, $\lambda_{cat} = e-1$ (Theorem [§4.4.5](/monograph/rules/dynamics/4.4/#4.4.5)).
$$
