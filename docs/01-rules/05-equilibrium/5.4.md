...
Assume the kinetic parameters satisfy the boundaries established by the **Global Stability** lemma <Ref id="5.4.3" label="§5.4.3" /> and the **Catalysis Bounds** lemma <Ref id="5.4.4" label="§5.4.4" />. Then a unique, non-zero equilibrium density $\rho^*$ exists and satisfies the transcendental balance equation. In particular, this fixed point constitutes an asymptotically stable attractor characterized by a strictly negative Jacobian eigenvalue $J < 0$.
...
1. **The Stability Criterion (**The Transcendental Balance** <Ref id="5.4.1" label="§5.4.1" />):** The dynamic instantiates the Jacobian derivative representation for the net flux function, establishing the mathematical conditions required for an asymptotic attractor.
2. **The Global Saturation (**Global Stability** <Ref id="5.4.3" label="§5.4.3" />):** The analysis evaluates the net flux function across asymptotic limits, proving that the creation current dominates at zero density while the deletion current dominates at high density.
3. **The Dissolution Threshold (**Catalysis Bounds** <Ref id="5.4.4" label="§5.4.4" />):** The constraint limits the space of admissible kinetic coefficients, ensuring that stress-induced cycle pruning does not outpace the autocatalytic creation potential of the vacuum.
...
Let $J_{\text{in}}$ and $J_{\text{out}}$ denote the creation potential and deletion potential, defined respectively by the quadratic approximations from the non-linear flux terms of the **Master Equation** <Ref id="5.2.7" label="§5.2.7" />:
$$
...
Substitution of the theoretical value established by the **Catalysis Coefficient Theorem.** <Ref id="4.4.5" label="§4.4.5" /> yields the relation:
$$
...
* **The Rarefaction Regime ($\rho > \rho^*$):** Excess geometric clustering generates localized topological tension, where the metric adjustments mediated by the **Entropic & Catalytic Decay** <Ref id="5.2.6" label="§5.2.6" /> process accelerate the deletion rate to clear out non-local shortcuts and restore metric sparsity.
* **The Inflationary Regime ($\rho < \rho^*$):** When density drops, catalytic stress vanishes and the erasure rate hits its linear floor, allowing the unhindered **Vacuum Permittivity** <Ref id="5.2.3" label="§5.2.3" /> constant $\Lambda$ and quadratic autocatalysis to act as a cosmic afterburner that re-ignites growth.
