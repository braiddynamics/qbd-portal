...
The friction function $f(\sigma) = \exp(-\mu \cdot \#\text{syndromes})$ with $\mu \approx 0.40$ [(§4.4.6)](thermo/monograph/foundations/dynamics#4.4.6) yields a damping factor $f_4 / f_3 \approx e^{-2\mu} \approx 0.67$ for the extra vertex exposure.
Combining factors:
