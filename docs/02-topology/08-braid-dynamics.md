...
The volume $V_\xi$ satisfies Ahlfors regularity $c_1 r^4 \leq |B(r)| \leq c_2 r^4$ [(§5.5.7)](/monograph/foundations/thermodynamics#5.5.7), with curvature bounds $|K(u,v)| \leq 2$ [(§5.5.4)](/monograph/foundations/thermodynamics#5.5.4). The finite substrate constraint $|U_{t_L}| < \infty$ [(§1.2.3)](/monograph/foundations/ontology/1.2/#1.2.3) ensures stability against fluctuations. The entropy scaling constant $c = \ln \Omega_{\mathrm{local}} / V_\xi > 0$ arises from the bounded degree $d_{\max}=3$ [(§5.5.3)](/monograph/foundations/thermodynamics#5.5.3). Central limit theorem damping over independent subregions yields a variance $\mathrm{Var}(\rho_3^*) \sim 1/N_\xi$, where $N_\xi = \frac{V_\xi}{\mathrm{vol}(\gamma)} \sim \rho_3^{*-3}$.
