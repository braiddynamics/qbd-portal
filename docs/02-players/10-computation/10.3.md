...
1. **The Syndrome Response** [(§10.3.3)](/monograph/players/computation/10.3/#10.3.3): The argument establishes that error syndromes trigger localized thermodynamic stress.
2. **The Code Distance** [(§10.3.4)](/monograph/players/computation/10.3/#10.3.4): The argument verifies that the minimum weight of an undetectable logical operator is three, ensuring code distance protection.
3. **The Thermodynamic Healing** [(§10.3.5)](/monograph/players/computation/10.3/#10.3.5): The argument proves that the evolution operator naturally deletes high-stress syndrome defects to restore the error-free codespace.
...
**I. Weight-1 Errors**
As proven in the **syndrome response lemma** [(§10.3.3)](/monograph/players/computation/10.3/#10.3.3), any single-qubit Pauli error $E$ on an edge $e$ anticommutes with at least one stabilizer $S \in \mathcal{S}$. Therefore, $E \notin N(\mathcal{S})$ (the normalizer). It is detectable. Distance $d > 1$.
...
**II. Error Detection**
A single-qubit error $E$ induces a syndrome flip $\sigma \to -1$ in the local neighborhood (the **syndrome response lemma** [(§10.3.3)](/monograph/players/computation/10.3/#10.3.3)). This creates a localized region of high stress (a "defect" or "quasiparticle").
...
* **Catalysis:** The high stress catalyzes the deletion kernel $\mathcal{Q}_{del}$ **the catalytic tension factor definition** [(§4.5.2)](/monograph/rules/dynamics/4.5/#4.5.2). The probability of deleting the erroneous edge is amplified ($f_{cat} > 1$).
...
Validation of the error detection capabilities established in the Minimum Weight the **weight analysis proof** [(§10.3.4.1)](/monograph/players/computation/10.3/#10.3.4.1) is based on the following protocols:
