...
The process $\mathcal{R}_X$ is a "writhe shuffle." It physically transforms the symmetric $(-1,-1,-1)$ braid into the asymmetric $(-2,-1,0)$ braid. It unties one loop and reties it elsewhere. This is a dramatic geometric change, yet under **Writhe Shuffling** <Ref id="10.4.1" label="§10.4.1" />, the total writhe (charge) is strictly conserved. It is like solving a Rubik's cube: you change the pattern (state) without peeling off the stickers (conserved quantities). This ensures the electron does not turn into a different particle while computing; it only changes its logical state.
