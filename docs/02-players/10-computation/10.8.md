...
1.  **Loop Nucleation:** A rewrite process that nucleates a temporary, closed 3-cycle loop adjacent to the target braid, adhering to the **geometric constructibility axiom** [(§2.3.1)](/monograph/rules/axioms/2.3/#2.3.1) by forming irreducible geometric quanta.
...
The **rewrite process definition** [(§10.8.1)](/monograph/players/computation/10.8/#10.8.1) implements this via "Self-Braiding." The qubit doesn't just sit there; it interacts with itself. A loop nucleates, winds around one of the qubit's ribbons, and annihilates. This process is a topological knotting event in spacetime, specifically a Dehn twist. It imparts a geometric phase (Aharonov-Bohm type) to the wavefunction. The precision of the $\pi/4$ phase comes not from analog tuning, but from the discrete topology of the winding number. It is a digital rotation enforced by the geometry of knots.
...
1. **The Category and Monoidal Foundations (the **category structures lemma** [(§10.8.3)](/monograph/players/computation/10.8/#10.8.3), the **monoidal structures lemma** [(§10.8.4)](/monograph/players/computation/10.8/#10.8.4)):** The argument establishes the category and monoidal structures of the braid system, proving consistent composition and system combination rules.
2. **The Braiding and Duality (the **braiding structures lemma** [(§10.8.5)](/monograph/players/computation/10.8/#10.8.5), the **duality structures lemma** [(§10.8.6)](/monograph/players/computation/10.8/#10.8.6)):** The argument verifies braiding and duality structures, validating exchange rules and matter-antimatter conjugates.
3. **The Twisting Morphism** [(§10.8.7)](/monograph/players/computation/10.8/#10.8.7): The argument instantiates the twisting morphism to construct the self-braiding operation.
4. **The Gate Action** [(§10.8.8)](/monograph/players/computation/10.8/#10.8.8): The argument combines these categorical structures to prove that the self-braiding operation implements the exact phase shift of pi over four required for the T-gate.
...
1.  **Tensor Product $\otimes$:** Disjoint union of graph supports (verified in the **monoidal structures lemma** [(§10.8.4)](/monograph/players/computation/10.8/#10.8.4)).
2.  **Braiding $\sigma$:** Particle exchange operation (verified in the **braiding structures lemma** [(§10.8.5)](/monograph/players/computation/10.8/#10.8.5)).
3.  **Duality $*$:** Particle-antiparticle pairing (verified in the **duality structures lemma** [(§10.8.6)](/monograph/players/computation/10.8/#10.8.6)).
4.  **Twist $\theta$:** Self-rotation (verified in the **twisting morphism lemma** [(§10.8.7)](/monograph/players/computation/10.8/#10.8.7)).
...
**I. Dual Object**
For a braid $\beta$ defined by writhe sequence $\{w_i\}$, the dual $\beta^*$ is defined by $\{-w_i\}$ with reversed **orientation strand reversed**[(§7.3.2)](/monograph/players/topology/7.3/#7.3.2).

**II. Evaluation and Coevaluation**
* **Coevaluation ($i_X: I \to X \otimes X^*$):** Pair creation from vacuum. $\mathcal{R}_{create}$ generates balanced writhe $\Delta w = 0$ **addition mode definition** [(§4.5.3)](/monograph/rules/dynamics/4.5/#4.5.3).
* **Evaluation ($e_X: X^* \otimes X \to I$):** Pair annihilation. $\mathcal{R}_{annihilate}$ removes the loop. This process is thermodynamically allowed as a $\sigma=+1$ stress-reducing process with $Q_{\text{del,thermo}}=1/2$ **the deletion probability theorem** [(§4.5.6)](/monograph/rules/dynamics/4.5/#4.5.6).
...
The operator anticommutes with the specific link stabilizer $L_S$ **twist anticommutation lemma** [(§7.1.3)](/monograph/players/topology/7.1/#7.1.3), enforcing non-trivial phase accumulation.
...
The **twist structure lemma** [(§10.8.7)](/monograph/players/computation/10.8/#10.8.7) verifies that rotating a particle by 360 degrees applies a specific phase factor. This allows the implementation of phase gates simply by rotating the particle in place. The **twist structure lemma** [(§10.8.7)](/monograph/players/computation/10.8/#10.8.7) ensures that this rotation is "natural"; it commutes with other operations in the correct way, linking the particle's spin to its computational utility.
...
Verification of the non-Clifford phase accumulation established in the **gate action proof** [(§10.8.8)](/monograph/players/computation/10.8/#10.8.8) is based on the following protocols:
...
1.  **Hadamard ($H$):** Implemented by the thermodynamic rewrite $\mathcal{R}_H$ **hadamard gate theorem** [(§10.6.1)](/monograph/players/computation/10.6/#10.6.1).
2.  **Controlled-Z ($CZ$):** Implemented by the catalytic bridge process $\mathcal{R}_{CZ}$ **controlled-z gate theorem** [(§10.7.1)](/monograph/players/computation/10.7/#10.7.1).
3.  **$\pi/8$ Phase Gate ($T$):** Implemented by the self-braiding process $\mathcal{R}_T$ **t-gate theorem** [(§10.8.2)](/monograph/players/computation/10.8/#10.8.2).
