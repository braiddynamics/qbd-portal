...
1. **The Crossing Scaling (Lemma [§6.3.4]):** The argument proves that crossing complexity scales linearly with the minimal crossing number, establishing the base mass contribution from braiding.
2. **The Torsional Scaling (Lemma [§6.3.5]):** The argument derives the quadratic scaling of torsional complexity, showing that twisting forces require a quadratic increase in geometric resources to satisfy causal partial ordering.
3. **The Entropic Vanishing (Lemma [§6.3.6]):** The argument proves that entropic fluctuations vanish for topologically protected states, allowing mass to be modeled strictly as a function of static complexity.
4. **The Synthesis (Proof [§6.3.7]):** The argument combines the linear crossing, quadratic torsional, and entropic parsimony results to synthesize the total topological mass functional.
...
Given that mass $m$ is defined as the informational inertia proportional to $N_3$ [(§7.4.1)](/monograph/players/topology/7.4/#7.4.1), it follows that mass scales linearly with the crossing number.
