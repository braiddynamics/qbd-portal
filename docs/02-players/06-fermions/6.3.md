...
1. **The Crossing Scaling** <Ref id="6.3.4" label="§6.3.4" />: The argument proves that crossing complexity scales linearly with the minimal crossing number, establishing the base mass contribution from braiding.
2. **The Torsional Scaling** <Ref id="6.3.5" label="§6.3.5" />: The argument derives the quadratic scaling of torsional complexity, showing that twisting forces require a quadratic increase in geometric resources to satisfy causal partial ordering.
3. **The Entropic Vanishing** <Ref id="6.3.6" label="§6.3.6" />: The argument proves that entropic fluctuations vanish for topologically protected states, allowing mass to be modeled strictly as a function of static complexity.
4. **The Synthesis** <Ref id="6.3.7" label="§6.3.7" />: The argument combines the linear crossing, quadratic torsional, and entropic parsimony results to synthesize the total topological mass functional.
...
Under the **Rewrite Rule $\mathcal{R}$** <Ref id="4.5.1" label="§4.5.1" />, this bridge forms via the closure of a compliant 2-path.
...
The operation $\mathcal{R}_{M+1}$ must satisfy **PUC Compliance** <Ref id="2.3.3" label="§2.3.3" />, which explicitly forbids the creation of redundant paths (bubbles) of length $\le 2$.
...
The **Architectural Stability Theorem** <Ref id="6.4.2" label="§6.4.2" /> establishes that prime braids are protected from local deformation by an $O(N)$ barrier.
