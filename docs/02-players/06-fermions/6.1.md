...
A decomposed cluster comprising isolated 3-cycles constitutes a high-stress configuration within the vacuum. The local cycle density $\rho_{\xi}$ strictly exceeds the equilibrium fixed point $\rho^*$ **equilibrium fixed point** [(§5.4.1)](/monograph/rules/equilibrium/5.4/#5.4.1), creating a syndrome potential that activates the non-linear deletion terms of the Master Equation. Specifically, the **Catalytic Flux** $J_{cat} = 3\lambda_{cat}\rho^2$ **catalytic flux relation** [(§5.2.7)](/monograph/rules/equilibrium/5.2/#5.2.7) dominates the dynamics, overpowering the friction-damped creation flux. The resulting net topological current is strictly negative ($\dot{\rho} \ll 0$), driving the rapid evaporation of the cluster back to the vacuum baseline.
...
Quantification of the density-dependent instability established in the Decay Rate Calculation the **decay rate calculation proof** [(§6.1.4.1)](/monograph/players/fermions/6.1/#6.1.4.1) is based on the following protocols:
...
The **Local Horizon** of the operator $\mathcal{R}$ is bounded by $R \sim \log N_{sys}$ **local horizon lemma** [(§6.4.3)](/monograph/players/fermions/6.4/#6.4.3).
