...
The **linear barrier definition** [(§6.4.1)](/monograph/players/fermions/6.4/#6.4.1) formalizes the concept of the **Topological Lock**. Imagine attempting to determine if a long rope is knotted by viewing it solely through a microscope with a restricted field of view. The observer sees only straight segments; the crossings that define the knot remain outside the frame. This scenario mirrors the predicament of the local rewrite rule $\mathcal{R}$. It operates within a logarithmic **scale horizon logarithmic**[(§2.7.2)](/monograph/rules/axioms/2.7/#2.7.2).
...
1. **The Local Horizon** [(§6.4.3)](/monograph/players/fermions/6.4/#6.4.3): The argument establishes that the local rewrite operator is confined to a logarithmic spatial horizon, rendering it blind to global topological structures.
2. **The Global Unwinding Barrier** [(§6.4.4)](/monograph/players/fermions/6.4/#6.4.4): The argument proves that untying an irreducible knot requires a coordinated sequence of operations scaling linearly with the system size, which cannot be initiated locally.
3. **The Stability** [(§6.4.5)](/monograph/players/fermions/6.4/#6.4.5): The argument synthesizes the scale separation between the logarithmic local horizon and the linear global barrier, proving that irreducible prime braids are stable against local vacuum decay.
...
The **Local Horizon** represents the maximum distance causal influence can propagate within a single update step. This radius scales logarithmically with the system size, $R \sim \log N$, acting as the "speed of light" limit for the graph's internal computation. The **local horizon lemma** [(§6.4.3)](/monograph/players/fermions/6.4/#6.4.3) establishes that any structure physically larger than $R$ remains effectively frozen to the rewrite rule.
...
1.  **Singular Transition (Tunneling):** Passing through the discriminant hypersurface $\Sigma$ where strands intersect.
    Cost: Infinite energy barrier due to **PUC** violation and **singularity graph** [(§6.4.1)](/monograph/players/fermions/6.4/#6.4.1).
...
By **the **reducibility lemma** [(§6.1.3)](/monograph/players/fermions/6.1/#6.1.3)**, there exists a local sequence $\mathcal{S}_{loc}$ of Type II/III moves that reduces the crossing number $C[\xi]$.
...
The **Catalytic Tension** $\chi(\sigma)$ **the catalytic tension factor definition** [(§4.5.2)](/monograph/rules/dynamics/4.5/#4.5.2) amplifies the deletion probability for the reducible components.
...
By **the **global unwinding barrier lemma** [(§6.4.4)](/monograph/players/fermions/6.4/#6.4.4)**, the cost of Global Unwinding is $O(N)$.
By **the **local horizon lemma** [(§6.4.3)](/monograph/players/fermions/6.4/#6.4.3)**, the local operator $\mathcal{R}$ is blind to the global gradient required to guide this $O(N)$ process.
