...
2.  **Degeneracy Analysis:** The **Principle of Unique Causality (PUC)** <Ref id="2.3.3" label="§2.3.3" /> enforces a rigid graph structure for the minimal embedding of a prime knot. Any local deviation constitutes a high-energy excitation (logical error) that triggers the **Hard Constraint Validity** <Ref id="3.5.4" label="§3.5.4" />.
