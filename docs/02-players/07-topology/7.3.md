...
3.  **Conservation Law:** The global value of $Q$ is a conserved quantity under all unitary evolution operators $\mathcal{U}$ **the evolution operator definition** [(§4.6.1)](/monograph/rules/dynamics/4.6/#4.6.1), enforced by the topological barriers against local writhe modification.
...
1. **The Writhe Conservation** [(§7.3.4)](/monograph/players/topology/7.3/#7.3.4): The argument proves that the total writhe of the tripartite braid remains invariant under local graph rewrites, establishing writhe as a conserved charge.
2. **The Lepton and Quark Spectrum (the **lepton and quark spectrum lemma** [(§7.3.5)](/monograph/players/topology/7.3/#7.3.5), the **quark spectrum lemma** [(§7.3.6)](/monograph/players/topology/7.3/#7.3.6)):** The argument derives integer charge values for symmetric singlet states and fractional charges for asymmetric triplet configurations.
3. **The Anomaly Normalization** [(§7.3.7)](/monograph/players/topology/7.3/#7.3.7): The argument fixes the charge scale factor to exactly one-third by enforcing the cancellation of gauge anomalies in the first generation.
4. **The Charge Emergence** [(§7.3.8)](/monograph/players/topology/7.3/#7.3.8): The argument combines these topological solutions to prove that electric charge is an emergent, quantized representation of total braid twist.
...
1.  **Local Blindness:** The Universal Constructor $\mathcal{R}$ operates within a bounded causal horizon $R \sim \log N$ **local horizon lemma** [(§6.4.3)](/monograph/players/fermions/6.4/#6.4.3), rendering it incapable of measuring global topological invariants such as the total winding number.
...
Let $\mathcal{O}_{loc}$ denote the set of all physically realizable operators generatable by the **Universal Constructor** $\mathcal{R}$ **the universal constructor definition** [(§4.5.1)](/monograph/rules/dynamics/4.5/#4.5.1).
The action of any operator $\hat{O} \in \mathcal{O}_{loc}$ restricts to a subgraph $G_{sub} \subset G$ defined by the **Local Horizon** radius $R \sim \log N$ **local horizon lemma** [(§6.4.3)](/monograph/players/fermions/6.4/#6.4.3).
...
Consider a macroscopic filament. A local observer viewing a small segment perceives the local twist but cannot count the *total* number of twists in the entire filament without traversing its length. Since the rewrite rule cannot traverse the particle instantaneously due to the **horizon causal** [(§6.4.3)](/monograph/players/fermions/6.4/#6.4.3), it remains blind to the total charge.
...
The **Hard Constraint Projectors** $\Pi_{cycle}$ **hard constraint validity lemma** [(§3.5.4)](/monograph/rules/architecture/3.5/#3.5.4) act on the state vector.
...
The **Evolution Operator** $\mathcal{U}$ **the evolution operator definition** [(§4.6.1)](/monograph/rules/dynamics/4.6/#4.6.1) includes the projection map $\mathcal{M}$.
...
The **Electric Charge Operator** $Q$ **the charge operator definition** [(§7.3.1)](/monograph/players/topology/7.3/#7.3.1) is defined linearly:
$$Q(\vec{w}) = \frac{1}{3} \sum_{i=1}^{3} w_i$$
The **Topological Complexity** $C(\vec{w})$ **topological mass theorem** [(§6.3.3)](/monograph/players/fermions/6.3/#6.3.3) scales with the absolute writhe sum (approximating crossing number scaling):
...
3.  **Stability Analysis (Parallelism):**
    By the **geometric sharing efficiency lemma** [(§7.4.5)](/monograph/players/topology/7.4/#7.4.5), parallel twists ($w_i, w_j > 0$) share geometric support structures within the causal graph (shared 3-cycles).
...
The **Total Writhe** $w(\beta)$ is established as a globally conserved quantum number under local evolution by the **writhe conservation lemma** [(§7.3.4)](/monograph/players/topology/7.3/#7.3.4).
The local dynamics are invariant under global writhe shifts by the **gauge symmetry lemma** [(§7.3.3)](/monograph/players/topology/7.3/#7.3.3), necessitating a $U(1)$ gauge field to enforce local covariance.
...
The **Anomaly normalization lemma** [(§7.3.7)](/monograph/players/topology/7.3/#7.3.7) proves that $k=1/3$ is the unique normalization satisfying the definition of the fundamental charge unit and anomaly cancellation.
...
1.  **Symmetric (Singlet) Sector:**
    Inputs: $w \in \{0, \pm 3\}$ (from the **lepton and quark spectrum lemma** [(§7.3.5)](/monograph/players/topology/7.3/#7.3.5)).
    Outputs: $Q \in \{0, \pm 1\}$.
    Matches: Neutrino ($0$), Electron ($-1$), Positron ($+1$).
2.  **Asymmetric (Triplet) Sector:**
    Inputs: $w \in \{-1, +2\}$ (from the **quark spectrum lemma** [(§7.3.6)](/monograph/players/topology/7.3/#7.3.6)).
