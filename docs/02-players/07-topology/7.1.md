...
1. **The Spin Identification** [(§7.1.1)](/monograph/players/topology/7.1/#7.1.1): The argument defines the spin operator on the ribbon rung edges, proving it measures twist parity and assigns eigenvalues of negative one to half-twisted fermionic states.
2. **The Twist Anticommutation** [(§7.1.3)](/monograph/players/topology/7.1/#7.1.3): The argument establishes that the unitary rotation operator anticommutes with the spin operator due to the odd number of edge flips required for a half-twist.
3. **The Exchange Equivalence** [(§7.1.4)](/monograph/players/topology/7.1/#7.1.4): The argument proves that the physical exchange of two identical braids is topologically equivalent to a local rotation of one braid by a full two-pi phase.
4. **The Phase Inversion** [(§7.1.5)](/monograph/players/topology/7.1/#7.1.5): The argument synthesizes the anticommutation and exchange equivalence properties to show that exchanging identical fermions introduces a negative sign in their joint wavefunction, recovering Fermi-Dirac statistics.
...
In the **formalism stabilizer** [(§3.5.1)](/monograph/rules/architecture/3.5/#3.5.1), each elementary rewrite maps to a Pauli-$X$ operation on a specific edge qubit.
...
The **Twist anticommutation lemma** [(§7.1.3)](/monograph/players/topology/7.1/#7.1.3) formalizes the interaction between a physical twist and the measurement of spin. The spin operator $L_S$ measures parity via a product of $Z$ operators. A physical twist, implemented by the unitary $\hat{\mathcal{T}}$, involves the creation and rearrangement of edges, actions that correspond to Pauli-$X$ operations in the qubit basis defined in the **Configuration Space Validity** [(§3.5.3)](/monograph/rules/architecture/3.5/#3.5.3).
...
Applying the eigenvalue result from the **eigenvalue inversion proof** [(§7.1.3.1)](/monograph/players/topology/7.1/#7.1.3.1):
...
In standard quantum mechanics, the Spin-Statistics Theorem constitutes a derived result requiring the axioms of relativity and causality. In Quantum Braid Dynamics, it exists as a topological tautology. The **Exchange equivalence lemma** [(§7.1.4)](/monograph/players/topology/7.1/#7.1.4) proves that exchanging two particles is geometrically identical to rotating one of them.
...
Each braid contains a set of rung edges defining the **Spin Stabilizers** $L_{S1}, L_{S2}$ **spin operator definition** [(§7.1.1)](/monograph/players/topology/7.1/#7.1.1).
...
By the **exchange equivalence lemma** [(§7.1.4)](/monograph/players/topology/7.1/#7.1.4), the exchange $\hat{P}_{12}$ applies a relative $2\pi$ twist to the pair.
