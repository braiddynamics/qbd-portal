...
2.  **Locality Constraint:** The **Principle of Unique Causality** <Ref id="2.3.3" label="§2.3.3" /> and the high energy barrier for non-local **Global Unwinding Barrier** <Ref id="6.4.4" label="§6.4.4" /> restrict the second fermion to the immediate neighborhood of $\{u, v\}$.
...
By the **Principle of Unique Causality (PUC)** <Ref id="2.3.3" label="§2.3.3" />, the creation of a path $u \to w \to v$ while $u \to v$ exists violates the **Principle of Unique Causality (PUC)**, triggering immediate deletion.
