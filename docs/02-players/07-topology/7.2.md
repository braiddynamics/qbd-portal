...
3.  **Axiomatic Violation:** The simultaneous activation of $(u, v)$ and $(v, u)$ constitutes a Directed 2-Cycle, which violates **Causal Primitive** [(§2.1.1)](/monograph/rules/axioms/2.1/#2.1.1) which enforces Asymmetry and **Acyclic Effective Causality** [(§2.7.1)](/monograph/rules/axioms/2.7/#2.7.1) which enforces a strict partial ordering.
...
1. **The Binary State** [(§7.2.2)](/monograph/players/topology/7.2/#7.2.2): The argument establishes that directed edges have a binary information capacity, preventing the stacking of multiple excitations on a single causal connection.
2. **The Forbidden Occupancy** [(§7.2.3)](/monograph/players/topology/7.2/#7.2.3): The argument demonstrates that attempting to place multiple fermions in the same state forces the creation of a directed two-cycle, directly violating the causal acyclicity constraint.
3. **The Exclusion Proof** [(§7.2.4)](/monograph/players/topology/7.2/#7.2.4): The argument proves that the hard constraint projector maps the dual-occupancy state to the null vector, rendering its physical probability zero and enforcing the Pauli exclusion principle.
...
The **Directed Causal Link** the **irreflexivity axiom** [(§2.1.1)](/monograph/rules/axioms/2.1/#2.1.1) defines the edge set $E$ strictly as a subset of the Cartesian product of the vertex set $V$.
...
The configuration space $\mathcal{H}$ is constructed via the mapping $\mathcal{M}: \Omega_{graph} \to (\mathbb{C}^2)^{\otimes K}$ **configuration space validity lemma** [(§3.5.3)](/monograph/rules/architecture/3.5/#3.5.3).
...
The **Finite Information Substrate** the **finite information density lemma** [(§1.2.3)](/monograph/rules/ontology/1.2/#1.2.3) bounds the information density of the graph.
...
Furthermore, the **Universal Evolution Operator** $\mathcal{U}$ **the evolution operator definition** [(§4.6.1)](/monograph/rules/dynamics/4.6/#4.6.1) acts via Pauli-$X$ bit-flips, which preserve the binary dimension.
...
2.  **Locality Constraint:** The **Principle of Unique Causality** [(§2.3.3)](/monograph/rules/axioms/2.3/#2.3.3) and the high energy barrier for non-local **connections** [(§6.4.4)](/monograph/players/fermions/6.4/#6.4.4) restrict the second fermion to the immediate neighborhood of $\{u, v\}$.
...
By the **binary state lemma** [(§7.2.2)](/monograph/players/topology/7.2/#7.2.2), the occupation limit of $e_{uv}$ is saturated ($n_{max}=1$).
...
By the **global unwinding barrier lemma** [(§6.4.4)](/monograph/players/fermions/6.4/#6.4.4), the formation of such a non-local structure constitutes a global topology change with an $O(N)$ energy barrier.
By the **principle of unique causality axiom** [(§2.3.3)](/monograph/rules/axioms/2.3/#2.3.3), the creation of a path $u \to w \to v$ while $u \to v$ exists violates the **Principle of Unique Causality (PUC)**, triggering immediate deletion.
...
The **Causal Primitive** the **irreflexivity axiom** [(§2.1.1)](/monograph/rules/axioms/2.1/#2.1.1) mandates strict Asymmetry:
...
The **Hard Constraint Projector** $\Pi_{\text{cycle}}$ **hard constraint validity lemma** [(§3.5.4)](/monograph/rules/architecture/3.5/#3.5.4) enforces the asymmetry axiom on the Hilbert space.
...
The probability of transitioning to the dual occupancy state is determined by the Born Rule applied to the projected evolution operator $\mathcal{U}$ **the evolution operator definition** [(§4.6.1)](/monograph/rules/dynamics/4.6/#4.6.1).
