...
1. **The Braid Isomorphism** <Ref id="8.1.2" label="§8.1.2" />: The argument establishes that discrete rewrite operations on particle ribbons correspond exactly to the algebraic generators of the braid group.
...
3. **The Commutator Depth** <Ref id="8.1.5" label="§8.1.5" />: The argument demonstrates that nested commutators of the braid generators close within a finite depth, defining a bounded Lie bracket structure.
4. **The Lie Algebra Emergence** <Ref id="8.1.6" label="§8.1.6" />: The argument combines these relations to prove that continuous gauge fields and Lie algebra generators emerge from the unitary evolution of the discrete braid graph.
...
3.  **Causal Consistency:** Both sequences satisfy the Acyclic Effective Causality the **acyclic effective causality axiom** <Ref id="2.7.1" label="§2.7.1" /> at every intermediate step, ensuring no forbidden causal loops are generated during the transformation.
...
2.  **Inductive Step:** At depth $d$, the nested bracket $[[\dots[H_i, H_{i+1}], \dots], H_{i+d}]$ generates connections spanning $d+1$ ribbons via commutators like $[H_i, H_{i+d-1}]$. The **Jacobi Identity** <Ref id="4.3.7" label="§4.3.7" /> ensures closure for associativity.
