...
1.  **Topological Distinctness:** Distinct reduced words (where no $\sigma_i \sigma_i^{-1} = 1$) yield minimal diagrams distinct up to isotopy (PUC prevents reducible Type II moves). The **Jones Polynomial** $V_\xi(t)$ [(§6.1.1)](/monograph/players/tripartite-braid#6.1.1) serves as the faithful invariant; since $\mathcal{R}$ sequences preserve the **Writhe** $w(\beta)$ and **Linking Matrix** $L_{ij}$ [(§6.1.1)](/monograph/players/tripartite-braid#6.1.1), distinct words map to graphs with distinct polynomial invariants.
...
2.  **Correlation Length:** At depth $d$, the nested bracket acts on a chain of $d+1$ ribbons. Locality bounds the support to $O(d)$ vertices via the **Correlation Length** $\xi \sim 1/\rho_e$ [(§5.5.5)](/monograph/rules/thermodynamics/5.5/#5.5.5).
3.  **BFS Search:** The search for PUC compliance scans the local ball $|B(R)| \sim R^4$ [(§5.5.7)](/monograph/rules/thermodynamics/5.5/#5.5.7) within radius $R = \log_{\text{diam}} N$. The detection of short-path alternatives occurs with probability $1 - e^{-R} \approx 1$ for $R = \log_3 10^6 \approx 9.5$.
