...
2.  **Monotonicity Enforcement:** The value of $\chi$ is fixed by the strict monotonicity of the History Function $H_t$ **monotonicity of history theorem** [(§1.3.4)](/monograph/rules/ontology/1.3/#1.3.4), where the forward causal order $H_t(e_1) < H_t(e_2)$ yields the left-handed value $\chi = -1$, and the reverse order yields the right-handed value $\chi = +1$.
...
1. **The Chiral Stability** [(§8.3.3)](/monograph/players/braids/8.3/#8.3.3): The argument establishes that the chiral invariant of a braid is preserved under standard evolutionary operations, securing handedness as a stable physical property.
2. **The Weak Doublet** [(§8.3.4)](/monograph/players/braids/8.3/#8.3.4): The argument constructs the weak doublet algebra from local swaps on paired lepton braids, establishing the emergent special unitary group of degree two symmetry.
3. **The Mirror Rejections (the **mirror path rejection lemma** [(§8.3.5)](/monograph/players/braids/8.3/#8.3.5), the **path rejection lemma** [(§8.3.6)](/monograph/players/braids/8.3/#8.3.6), the **mirror path rejection lemma** [(§8.3.7)](/monograph/players/braids/8.3/#8.3.7)):** The argument demonstrates that right-handed paths are rejected by the evolution comonad because their mirror transitions violate the Principle of Unique Causality.
4. **The Chiral Weak Current** [(§8.3.8)](/monograph/players/braids/8.3/#8.3.8): The argument integrates the doublet algebra and parity-violating path constraints to derive the chiral vector-minus-axial electroweak current.
...
1.  **Functorial Preservation:** The evolution of the graph constitutes a functor in the History Category $\mathbf{Hist}$ **categorical ties to prior foundations commentary** [(§4.1.3)](/monograph/rules/dynamics/4.1/#4.1.3), which preserves the partial ordering of edges $e_a \le e_b$ under all valid morphisms.
...
1.  **Functoriality:** The evolution defines a functor in the **History Category** $\mathbf{Hist}$ **categorical ties to prior foundations commentary** [(§4.1.3)](/monograph/rules/dynamics/4.1/#4.1.3). Morphisms $f: G \to G'$ map edges $e$ to $f(e)$ while preserving the partial order: $e_a \le e_b \implies f(e_a) \le f(e_b)$.
...
2.  **Generator 2:** Let $\hat{H}_z$ be the operator measuring the writhe difference (Hypercharge/Isospin projection). In the doublet basis, this arises from the **Spin Stabilizer** $L_S$ **spin operator definition** [(§7.1.1)](/monograph/players/topology/7.1/#7.1.1):
...
The emergence of SU(2) **weak doublet algebra lemma** [(§8.3.4)](/monograph/players/braids/8.3/#8.3.4) parallels the $SU(3)$ derivation but applies it to the electroweak doublet. The rewrite process, which swaps the neutrino and electron braid topologies, acts as the generator of the weak force.
...
The chiral invariant $\chi$ **chiral stability lemma** [(§8.3.3)](/monograph/players/braids/8.3/#8.3.3) embeds a left-handed preference via the timestamp ordering $H_t$.
...
The violation probability is governed by the correlation length $\xi \sim 1/\rho_e$ **correlation decay lemma** [(§5.5.5)](/monograph/rules/equilibrium/5.5/#5.5.5):
...
The electroweak doublet $(\nu_e, e^-)_L$ is embedded in the tripartite braid as the subspace of **Writhe-Neutral** **states** [(§7.3.5)](/monograph/players/topology/7.3/#7.3.5).
Basis: $|\nu_e\rangle$ ($w=0, \lambda=(1,1,1)$) and $|e^-\rangle$ ($w=-3, \lambda=(-1,-1,-1)$).
These states are mixed by $\mathcal{R}_W$ via rung shuffles on the shared 3-cycle **weak doublet algebra lemma** [(§8.3.4)](/monograph/players/braids/8.3/#8.3.4).
The operator $\mathcal{R}_W$ acts as $\sigma_x$, flipping between the states while conserving Total Charge $Q = w/3$ **the charge operator definition** [(§7.3.1)](/monograph/players/topology/7.3/#7.3.1) modulo the weak mixing angle.
...
This imposes the chiral sign $\chi = -1$ **chiral stability lemma** [(§8.3.3)](/monograph/players/braids/8.3/#8.3.3).
The acceptance weight for the rewrite is biased by $e^{\chi \mu \cdot \text{stress}}$ **the catalytic tension factor definition** [(§4.5.2)](/monograph/rules/dynamics/4.5/#4.5.2).
...
The **Correction Map** $C$ projects this state out with probability $\approx 1$ **mirror path rejection lemma** [(§8.3.7)](/monograph/players/braids/8.3/#8.3.7).
...
The generators $\hat{H}_{x,y,z} \propto \sigma_{x,y,z}$ **weak doublet algebra lemma** [(§8.3.4)](/monograph/players/braids/8.3/#8.3.4) act exclusively within the $\chi = -1$ subspace.
