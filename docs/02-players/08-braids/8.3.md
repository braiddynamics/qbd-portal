...
1. **The Chiral Stability (Lemma [§8.3.3]):** The argument establishes that the chiral invariant of a braid is preserved under standard evolutionary operations, securing handedness as a stable physical property.
2. **The Weak Doublet (Lemma [§8.3.4]):** The argument constructs the weak doublet algebra from local swaps on paired lepton braids, establishing the emergent special unitary group of degree two symmetry.
3. **The Mirror Rejections (Lemma [§8.3.5], Lemma [§8.3.6], Lemma [§8.3.7]):** The argument demonstrates that right-handed paths are rejected by the evolution comonad because their mirror transitions violate the Principle of Unique Causality.
4. **The Chiral Weak Current (Proof [§8.3.8]):** The argument integrates the doublet algebra and parity-violating path constraints to derive the chiral vector-minus-axial electroweak current.
...
2.  **Generator 2:** Let $\hat{H}_z$ be the operator measuring the writhe difference (Hypercharge/Isospin projection). In the doublet basis, this arises from the **Spin Stabilizer** $L_S$ [(§7.1.1)](/monograph/players/topology/7.1/#7.1.1):
...
The violation probability is governed by the correlation length $\xi \sim 1/\rho_e$ [(§5.5.5)](/monograph/rules/equilibrium/5.5/#5.5.5):
...
The operator $\mathcal{R}_W$ acts as $\sigma_x$, flipping between the states while conserving Total Charge $Q = w/3$ [(§7.3.1)](/monograph/players/topology/7.3/#7.3.1) modulo the weak mixing angle.
