...
1. **The Friction Ratio (Lemma [§8.4.2]):** The argument establishes the mathematical bounds on rewrite friction, proving that triangle-closing operations encounter less resistance than square-closing ones.
2. **The Coupling Correspondence (Lemma [§8.4.3]):** The argument demonstrates that gauge couplings map directly to rewrite transition probabilities via the Born amplitude rule.
3. **The Complexity Identification (Lemma [§8.4.4]):** The argument maps weak isospin to three-cycle flips and weak hypercharge to four-cycle phase shifts, identifying their respective geometric complexities.
4. **The Angle Synthesis (Proof [§8.4.5]):** The argument combines the friction, coupling, and complexity identities to prove the exact value of the electroweak mixing angle.
...
At the equilibrium density $\rho^* \approx 0.029$ with friction $\mu \approx 0.40$ [(§4.4.6)](/monograph/rules/dynamics/4.4/#4.4.6), this factor evaluates to $e^{-0.0232} \approx 0.977$.
