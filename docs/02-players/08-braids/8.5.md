...
4.  **Vacuum Density:** The value $\rho_3^* \approx 0.029$ represents the equilibrium concentration of compliant rewrite sites in the causal graph, as determined by the fixed point of the Master Equation [(§5.4.1)](/monograph/rules/thermodynamics/5.4/#5.4.1).
...
The causal graph embeds into a 4-dimensional manifold (Ahlfors regularity dimension $d_c = 4$) [(§5.5.7)](/monograph/rules/thermodynamics/5.5/#5.5.7).
