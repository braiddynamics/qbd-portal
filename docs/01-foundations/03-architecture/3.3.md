...
Consider $s_a \in S'$ and $s_b \in \mathcal{S}_{sites} \setminus S'$. The vacuum state $G_0$ is a vertex-transitive and site-transitive graph [(§3.2.1)](#3.2.1). There exists $\sigma \in \text{Aut}(G_0)$ such that $\sigma(s_a) = s_b$.
...
Assume the graph remains in the sparse regime [(§3.1.2)](#3.1.2) subject to quasi-local constraints [(§2.3.3)](/monograph/foundations/axioms/2.3/#2.3.3) with a bounded check radius $R \propto \log N$. Then the time complexity of the maximally parallel update operation is bounded by $O(\log N)$. Moreover, the probability of conflict chains spanning the system decays exponentially.
