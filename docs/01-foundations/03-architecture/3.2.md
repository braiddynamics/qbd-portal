...
For any non-regular tree graph, candidacy for the vacuum state $G_0$ is excluded by the requirement for maximal orbit entropy [(§3.2.9)](/monograph/foundations/architecture/3.2/#3.2.9).
...
Investigation of the entropic properties of regular versus irregular structures established in the Regularity Mandate Proof [(§3.2.7.1)](/monograph/foundations/architecture/3.2/#3.2.7.1) is based on the following protocols:
...
For any tree graph where the automorphism group fails to act transitively on vertex levels, candidacy for the vacuum state $G_0$ is excluded by the condition of relational uniformity [(§3.2.9)](/monograph/foundations/architecture/3.2/#3.2.9). In particular, level-transitivity constitutes a necessary condition for the absence of privileged positions within each generation.
...
The Regular Bethe Fragment [(§3.2.1)](/monograph/foundations/architecture/3.2/#3.2.1) constitutes the unique maximizer of the Structural Optimality Score $\mathcal{O}(G; \lambda)$ over the class of axiomatically admissible graphs for the parameter range $\lambda \in [0.4, 0.6]$.
...
The class of axiomatically admissible graphs reduces, through the cumulative exclusions of the previous lemmas, to the singleton containing the **Regular Bethe Fragment** [(§3.2.1.1)](/monograph/foundations/architecture/3.2/#3.2.1.1) with internal coordination number $k_{deg} \ge 3$.
...
Sequential application of the lemma filters and explicit computation of the **Structural Optimality Score** [(§3.2.9)](/monograph/foundations/architecture/3.2/#3.2.9) confirms the maximum.
...
Verification of the Regular Bethe Fragment as the unique maximizer established in the Supremacy Verification Proof [(§3.2.10.1)](/monograph/foundations/architecture/3.2/#3.2.10.1) is based on the following protocols:
...
Quantification of the scaling behavior of the Bethe fragment established in the Regularity Mandate Proof [(§3.2.7.1)](/monograph/foundations/architecture/3.2/#3.2.7.1) is based on the following protocols:
...
:::tip[**Formal Derivation of the Regular Bethe Fragment (k=3) from the Intersection of Constraints [(§3.2.1)](/monograph/foundations/architecture/3.2/#3.2.1)**]
:::
...
2.  **Interaction Maximization (Lemma [§3.2.6](/monograph/foundations/architecture/3.2/#3.2.6)):** To maximize the rate of geometric evolution, the tree structure must maximize the density of compliant 2-path sites per vertex. This requirement favors maximal branching over linear extension.
3.  **Symmetry Maximization (Lemma [§3.2.8](/monograph/foundations/architecture/3.2/#3.2.8)):** To prevent the emergence of privileged spatial locations or preferred directions, the graph must exhibit **Level Transitivity** in its automorphism group. This enforces structural regularity, requiring $k$ to be constant across all internal nodes.
