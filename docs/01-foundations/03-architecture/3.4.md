...
Let $G_0 = (V, E_0)$ denote the vacuum state. The **Depth-Parity Bipartition** [(§3.1.10)](#3.1.10) establishes that $G_0$ admits a canonical 2-coloring:
...
Let $G_1$ denote the state immediately following the tunneling event $e_{\text{tunnel}} = (u, v)$ where $u, v \in V_{\text{even}}$. The underlying structure of $G_0$ constitutes a Maximally Branched Tree [(§3.2.6)](#3.2.6). Consequently, the internal vertex $v$ possesses an out-degree $k \ge 1$:
...
1.  **Direct Edge $(u, w)$:** $E_0$ contains only even-odd edges. While the parities permit a connection, the tree structure of $G_0$ implies a unique path between any two nodes. A direct edge would create a triangle $(u, v, w)$, violating **Acyclicity** [(§3.1.7)](#3.1.7). Thus $(u, w) \notin E_0$.
2.  **Alternative 2-Path:** Any other path implies a cycle in the underlying undirected graph, violating the **Tree Condition** [(§3.1.9)](#3.1.9).
