...
The base thermodynamic acceptance probability for edge creation, denoted $\mathbb{P}_{\text{acc,thermo}}$, is identically equal to 1 in the critical vacuum regime. This unity probability is a consequence of the barrierless free energy condition ($\Delta F < 0$) derived from the bit-nat equivalence [(§4.4.1)](/monograph/foundations/dynamics/4.4/#4.4.1).
...
From the **Thermodynamic Foundations** [(§4.4)](/monograph/foundations/dynamics/4.4/#4.4), the creation of a geometric quantum (3-cycle) entails:
...
Consider the case with finite energy cost $\epsilon_{geo} = \frac{\ln 2}{4}$ [(§4.4.4)](/monograph/foundations/dynamics/4.4/#4.4.4).
