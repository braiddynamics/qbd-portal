...
Let the Geometric Quantum $\gamma$ denote the subgraph induced by the ordered triplet of vertices $(u, v, w)$ such that the edge set contains exactly $\{(u, v), (v, w), (w, u)\}$. Then this structure constitutes the minimal closed cycle compatible with the Causal Primitive [(§2.1.1)](/monograph/foundations/axioms/2.1/#2.1.1), excluding cycles of length 1 and 2, and the set of all $\gamma \subset G$ constitutes the basis for emergent spatial area.
...
The case $L=1$ implies an edge $e = (u, u)$. This configuration is excluded by Axiom 1 (Irreflexivity) [(§2.1.1)](/monograph/foundations/axioms/2.1/#2.1.1):

$$
(u, u) \notin E \implies L \neq 1
$$

The case $L=2$ implies edges $e_1 = (u, v)$ and $e_2 = (v, u)$ with $u \neq v$. This configuration is excluded by Axiom 1 (Asymmetry) [(§2.1.1)](/monograph/foundations/axioms/2.1/#2.1.1):
...
Let $\Phi(G)$ denote the Lexicographic Potential of a finite graph $G$ [(§2.3.4)](/monograph/foundations/axioms/2.3/#2.3.4). Then the codomain of $\Phi$ is well-ordered, and any trajectory $G_0, G_1, \dots$ satisfying the descent condition $\Phi(G_{t+1}) < \Phi(G_t)$ constitutes a finite sequence.
