...
4.  **Evolution and Aggregation:** The system is advanced via 1500 iterative applications of the Evolution Operator $\mathcal{U}$ [(§4.6.1)](/monograph/foundations/dynamics/4.6/#4.6.1). Observables (specifically $N_3$ and $\rho_3$) are recorded at each tick, and statistical moments (mean, median, skew) are aggregated across the ensemble.
